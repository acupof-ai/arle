...
| `ARLE_CUDA_ENABLE_FA3` | — | deleted 2026-08-05 | Was a build-time opt-in for the vendored FA3 hopper hdim256/bf16/sm90 units. FA3 now builds whenever the vendored tree (`crates/cuda-kernels/vendor/flash-attention/`) and an sm_90 target are both present; there is nothing to set. A build without sm_90 links `arle_fa3_stubs.cu`, `--qwen35-fa3` becomes a no-op, and serve logs `FA3 stub build` at `warn`. |
