...
| `ARLE_CUDA_ENABLE_DEEPGEMM_NATIVE` | `1` / unset | unset | Build-time switch for the optional raw-pointer DeepGEMM C ABI bridge. Requires CUDA driver/runtime libs and DeepGEMM/CUTLASS sources; runtime JIT uses `${CUDA_HOME}/bin/nvcc` plus `cuobjdump`, so the CUDA bin tools must be present. Without this switch the required `deepgemm` backend fails preflight before serving. `ARLE_CUDA_ENABLE_DEEPGEMM_TORCH=1` is accepted as a compatibility alias but no longer links PyTorch. |
