...
| `ARLE_DSV4_EXPERT_BACKEND` | `native`, `deepgemm-auto`, `deepgemm` | runtime: `deepgemm-auto`; `scripts/dsv4_toolchain.sh`: `deepgemm` | Selects the DSv4 local expert backend. `native` keeps the current per-expert/raw grouped GEMV paths. `deepgemm-auto` preflights the native DeepGEMM JIT dependencies during model load, builds the resident FP8 expert-weight cache only when the bridge is usable on SM90, and logs the exact fallback reason before the first request. `deepgemm` makes that bridge required and fails fast on build/runtime incompatibility; DSv4 validation scripts use this mode by default. |
