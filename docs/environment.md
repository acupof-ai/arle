...
| `ARLE_NVCC_WRAPPER` | command | unset | Optional wrapper for every main CUDA `.cu` compilation in `crates/cuda-kernels/build.rs`. Typical value: `sccache`, which runs `sccache /usr/local/cuda/bin/nvcc ...`. DeepEP sidecar compilation is not wrapped by this knob yet. |
| `ARLE_NVCC_SPLIT_COMPILE` | integer | unset | Optional `nvcc --split-compile=<N>` value for main CUDA `.cu` compilation. Use a bounded value such as `8` or `16` on high-core build hosts; unset preserves the current nvcc behavior. |
