...
| `ARLE_DSV4_EXPERT_BACKEND` | `native`, `deepgemm-auto`, `deepgemm` | `deepgemm` | Selects the DSv4 local expert backend. The runtime default is required DeepGEMM: the native DeepGEMM JIT bridge and resident FP8 expert-weight cache must preflight successfully, or DSv4 fails before serving instead of silently falling back to native grouped GEMV. `deepgemm-auto` remains an explicit debug/rollback mode that logs the exact fallback reason before the first request. `native` keeps the current per-expert/raw grouped GEMV paths for controlled diagnosis only. |
