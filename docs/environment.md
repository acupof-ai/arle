...
| `ARLE_DEEPGEMM_ROOT` | path | `crates/deepseek-kernels-sys/vendor/deepgemm` | Build-time DeepGEMM source root for the optional native bridge. Use a recursive upstream clone when the vendored third-party submodules are not populated. |
...
| `ARLE_CUDA_KERNELS_PREBUILT_DIR` | path | unset | Build-time fast path for CUDA kernel artifacts. The directory must contain `libkernels_cuda.a`, `libdeepseek_kernels.a`, and `libtilelang_kernels_aot.a` (producer manifest schema 4); when set, `crates/deepseek-kernels-sys/build.rs` links `libdeepseek_kernels.a`, `crates/cuda-kernels/build.rs` links the other two and hash-verifies all three, and both skip all `nvcc` and TileLang AOT work. If the directory also contains `arle_deepep_sidecar`, the sidecar path is baked into the binary. Keep the artifact key tied to CUDA toolkit, SM list, feature flags, TileLang version, DeepGEMM root, and the `crates/cuda-kernels` + `crates/deepseek-kernels-sys` source hash. Produce a pack from a built target tree with `scripts/export_prebuilt_cuda_kernels.sh <dest>`; the consumer key is `arle-cuda-kernels.manifest`, with `manifest.json` kept as human-readable provenance. |
...
### Lever gate harness (`scripts/lever_gate.sh`)
...
| `CONCURRENT_CONC` | `4` | Concurrent arm: in-flight requests |
