...
| `train` | Runtime-led post-training substrate (teacher via `infer-api`, student LoRA), train-side `/v1/train/*` control plane, shared async observability. Pretrain / SFT / GRPO / multi-turn retired 2026-05-18 — see `docs/projects/2026-05-18-opd-only-pivot.md`. **Two objective families** share this substrate (rollout → score → LoRA backward): **(a) OPD** (`opd` / `self-opd`) — teacher/EMA logits + KL distill; **(b) RFT / rejection-sampling** (`agent-opd` / `rubric-opd`) — execution or rubric-judge reward selects trajectories, trained by completion/response-masked CE (no teacher forward, no KL). "OPD-only" is the runtime-led *positioning*; `agent`/`rubric` are on-policy RFT built on the same loop, not distillation. | GPU kernels, scheduler |
