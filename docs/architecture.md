...
| `infer-cuda` | CUDA `BackendExecutor` + KV pool: paged KV, TileLang AOT + native-CUDA kernels, TP/EP, DeepGEMM, DeepEP, DSv4-Flash, GLM-5.2 (`glm_moe_dsa`, on the DSv4 path; forward landed, verification pending-remote), Qwen3.5/3.6 hybrid+MoE (FP8 MoE via DeepGEMM, batched paged decode), Qwen3.8 NVFP4 (Marlin W4A16 `marlin_fp4_gemm`, repacked at load; prefill m≥512 dequantizes to FP8 for DeepGEMM), DSv4 whole-step decode CUDA graph (default-on since 2026-08-23, `1a48d179f`; `ARLE_DSV4_DECODE_GRAPH=0` is the eager arm), over `cuda-kernels` | Scheduler logic, HTTP, terminal UX |
