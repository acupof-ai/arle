...
| `infer-quant` | Host-only weight-quant checkpoint ABI: `QuantFormat`, `QuantManifest`, `detect_quant_format`, scale-shape validation, FP8 E4M3 codec, NVFP4 CPU reference (`cpu_ref_fp4`). Zero device calls. | Kernels, device dequant math, scheduler |
