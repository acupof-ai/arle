...
| `infer-cuda` | CUDA `BackendExecutor` + KV pool: paged KV, TileLang AOT + native-CUDA kernels, TP/EP, DeepGEMM, DeepEP, DSv4-Flash, GLM-5.2 (`glm_moe_dsa`, on the DSv4 path; forward landed, verification pending-remote), Qwen3.5/3.6 hybrid+MoE (FP8 MoE via DeepGEMM, batched paged decode), over `cuda-kernels` | Scheduler logic, HTTP, terminal UX |
...
| `cuda-kernels` | CUDA kernel layer (`csrc/`, TileLang AOT, Rust FFI, paged-KV / TileLang metadata / graph-pool / tensor / kv_quant) | Model code, scheduler logic, tokenizer |
...
| `infer-cuda` | CUDA executor as a thin seam impl over `cuda-kernels`: paged KV, TileLang AOT + native-CUDA kernels, TP/EP, DeepGEMM, DeepEP, DSv4-Flash, GLM-5.2 (DSv4 path, verification pending-remote), Qwen3.5/3.6 hybrid+MoE (FP8 MoE via DeepGEMM). |
