...
> NOT config. Fix shipped: `--rollout-temperature 0.3` (`2394a2ab0`). Full
> forensics + the five-hypothesis false-chain in the errors entry.
...
- **Open (cheap):** logprob spot-check — is the occasional non-ASCII token at
  temp=1.0 a top_p=0.95 nucleus leak (CUDA sampler bug) or genuine model tail?
  Either way temp=0.3 stands.
