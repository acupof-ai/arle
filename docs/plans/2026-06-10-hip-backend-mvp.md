...
| Vulkan executor lane | **KEEP (accepted alternate — ckl 2026-06-10 "vulkan 也可以")** | llama.cpp Vulkan is competitive on this box (pp512 881 vs HIP+hipBLASLt ~986 t/s) with zero ROCm fragility, and one Vulkan executor would cover AMD/Intel/NVIDIA consumer AIPCs + Windows. Cost: shader corpus (GLSL→SPIR-V, KHR_cooperative_matrix on RDNA3.5) is farther from our CUDA idioms than HIP. **MVP lane stays HIP** (shortest port from `infer-cuda` kernels); lane choice gets a day-1 on-box license: llama.cpp ROCm-vs-Vulkan A/B on the same box (§4 checklist) — if ROCm fragility bites, Vulkan is the sanctioned pivot, borrowing `ggml/src/ggml-vulkan` shader patterns |
