...
Date: 2026-08-23
Scope: `crates/**/*.rs`, production code only. Totals include sites in crates that are no longer part of this repo; the tables list current sites only. Excluded: `#[cfg(test)]` modules and test-path files (`tests/`, `test_*.rs`, `*_test.rs`, `ffi/gemm_tests.rs`), and five files under concurrent modification (`infer-metal/src/{executor,dflash,lfm2,weights}.rs`, `mlx-sys/src/lib.rs`).
...
| 3. Unsafe trait impls | 21 | Yes, one to verify | All `Send`/`Sync` for FFI handle types; each carries a SAFETY comment |
| 4. `static mut` access | 0 | — | None in the codebase |
| 5. `transmute` | 1 block (2 call sites) | Yes | `dlsym` result to NVTX fn pointers; standard pattern |
| 6. Other | 61 blocks | Yes | `env::set_var` (13), `mmap` (7), uninitialized device alloc / `MaybeUninit` / `align_to` (41) |

No unsound unsafe code was found. Two items need verification or comment fixes (Recommendations 1–2); the rest are cosmetic.
...
| cudarc driver/runtime | `infer-cuda/src/{attention,graph,tp}.rs` | cudarc `Result`; stream-ordered on the engine thread's stream |
| NCCL | `cuda-kernels/src/collective.rs`, `infer-cuda/src/tp.rs` | `nccl::check` |
| MLX C bridge (`mlx_sys::*`) | `infer-metal/src/{mlx,qwen35,deepseek_ocr}.rs` | `panic_if_mlx_error` / `mlx_last_error` after each call; `mlx_guard()` serializes calls |
| xgrammar / DeepEP | `xgrammar-sys/src/lib.rs`, `deepep-sys/src/lib.rs` | status codes checked |
| libc (dlopen, pipe, flock, affinity, kill) | `kv-native-sys/`, `cli/serve_multiproc.rs`, `infer-cuda/src/{numa_pin,loader}.rs` | return codes checked; fds owned after success |

Top files: `infer-cuda/src/attention.rs` (75), `cuda-kernels/src/attention.rs` (63), `cuda-kernels/src/moe.rs` (53 blocks + 47 `unsafe fn`), `infer-metal/src/mlx.rs` (43), `infer-metal/src/qwen35.rs` (42).
...
| `ptr::copy_nonoverlapping` | `safetensors_io.rs`, `dsv4/slot_image.rs` (2) | Length derived from source and target element counts; source and target are distinct allocations |
| Pointer arithmetic (`.add()`) | `cuda-kernels/src/tensor/weight_format.rs` (3) | Offset bounds-checked against the buffer length before the add (SAFETY comments cite the check) |
...
Top files: `infer-cuda/src/qwen35_state.rs` (8), `infer-cuda/src/loader.rs` (7), `cuda-kernels/src/tensor/weight_format.rs` (6), `cuda-kernels/src/tensor/device_matrix.rs` (5).
...
### 5. `transmute` — 1 block, 2 call sites

`infer-cuda/src/nvtx.rs:65-66` transmutes `dlsym` results to `Option<fn(i32) -> i32>` / `Option<fn() -> i32>` for `nvtxRangePushA` / `nvtxRangePop`. The signatures match the NVTX C ABI (documented in the SAFETY comment); pointers are null-checked before use. This is the standard `dlsym` pattern; `std` provides no safe alternative.
...
3. **`cuda-kernels/src/kv_quant.rs:175` — drop the unsafe block.** `paged_attention_quantized_fa3_workspace_bytes` is a pure host-side size computation; the block exists only because the extern declaration is `unsafe`. A safe wrapper in the FFI module removes it. Cosmetic.

4. **`cuda-kernels/src/tensor.rs:92,209` — redundant inner block.** Inside `unsafe fn uninit`, the inner `unsafe { ctx.stream.alloc(...) }` wraps a safe cudarc call. The block documents the boundary, which is acceptable, but it is not required for soundness.

5. **Safe alternatives where they fit:**
...
6. **Test-only note:** `infer-cuda/src/executor.rs:958` mutates process-global env inside a `#[cfg(test)]` test. It is safe today (no other env-mutating tests run in parallel in that crate) but will race if the test suite parallelizes.
