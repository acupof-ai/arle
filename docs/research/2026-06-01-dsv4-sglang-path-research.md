...
|---|---|---|---|---|
| Process model | one process per rank under SGLang distributed runtime | ARLE has multiproc scaffolding and targeted relay envelopes; DSv4 startup still selects the replicated-token lane by default | Different | Native DeepEP can boot, but the production-selected request lane still does not match SGLang |
...
| Request ownership | DP/attention and MoE groups own distinct row shards where configured | ARLE has a guarded token-owned control plane: selected owner groups, targeted relay, remote visible-output return, and explicit request token-sync NCCL capability; DSv4 startup still does not select it, and axis-derived subgroup NCCL/data-plane execution are incomplete | Different | The old full-rank broadcast assumption is being deleted, but the SGLang owner path is not yet a performance path |
