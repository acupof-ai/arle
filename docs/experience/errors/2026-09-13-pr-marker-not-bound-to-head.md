...
code-identical head". The head's parent was main, which did not carry the
change, so the phrase named no commit. Reading the lane's reflog, exactly one
other commit carried a byte-identical blob, and the controller wrote that one
into the body as the build commit. It was wrong: the author had built three
amends further back, at a commit whose blob is *not* byte-identical.

Byte-identity of the blob was the wrong instrument. What matters is whether the
...
git show <build-commit>:<file> | strip > a.txt
git show <head>:<file>         | strip > b.txt
diff a.txt b.txt | grep -c '^[<>]'      # must be 0
```

On #446 this returned 0 differing lines for all three candidate commits over
564 code lines, and 107 for the lane's base commit, which is the positive
control that proves the comparison discriminates. Without that control a
stripper with a bug returns 0 for everything, which is the false-zero shape
three separate detectors took earlier the same day.

So option 1 does not need the marker to name the head. It needs the marker to
name *a* commit, and the check to compare that commit's compiled content
against the head's. That is strictly weaker to satisfy and strictly stronger as
evidence than an unverified claim that two commits are "comment-only" apart --
which is what both the author and the controller asserted here, each from a
different wrong premise, before anyone ran the comparison.

A note for whoever implements it: lane shas do not survive a squash merge, so
the check has to run before the merge, and an entry written afterwards must
cite the PR number instead. This section originally quoted four lane shas and
the dead-sha hygiene rule refused the push, correctly.
