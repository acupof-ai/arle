# RMSNorm convention flip — TWO bugs in one commit, both fixed

> Status: ROOT-CAUSED + FIXED. carried **two independent** wrong
> changes; the fix landed in two commits. **Type-A**: the kernel half
...
> → temp=1.0 sampled-tail garbage (greedy survives). fixed only the
...
degenerate output. A prior session's relay (#48) said " breaks temp>0" —
**correct from the start.** The chase below spent eight hypotheses re-discovering
that, because passed its own smoke (short greedy needle/Fibonacci
...
## Root Cause
 ...
 ("hd256 q/k RMSNorm convention mismatch — OFFSET→STANDARD") changed
...
's smoke passed), long context degenerates (agentic drift, temp>0 salad).

**Binary bisect — clean adjacent flip** (base Qwen3.6-27B-FP8, greedy agentic
cc-harness rollout; pre- the rollout is greedy by default):

| commit | rollout |
|---|---|
| (= ^, still `(1+w)`) | GOOD — 5 turns, `<tool_call>` Grep/Read/Bash, fixed the sqlparse bug, reward 1.0 |
| **** (`w`) | BAD — degenerate salad, zero tool calls |
| (= ^) | BAD (so is exonerated for THIS bug) |
...
## Fix
 ...
 — restore OFFSET `(1+weight)` at all 5 hd256 q/k sites
...
`fused_attention.cu`). 27B-only; 4B/hd128 untouched; keeps 's separate
(correct) MTP `pre_fc_norm` load fix.

**Verified on pod** (isolated build, sm_90 nvcc rebuild, HEAD): base
...
## Type-B — a SEPARATE temp>0 defect (#167): ROOT-CAUSED + FIXED
 ...
 carried **two** wrong changes reverted only the kernel
one. The Rust half survived at HEAD and is a distinct temp>0 corruption:

1. **Type-A = 's 4 `.cu` sites** (hd256 q/k RMSNorm OFFSET→STANDARD) —
 broke greedy-at-length AND temp>0; **fixed by **. Garbage
   `funciton/Fibonaacci/_selection_selection_`.
2. **Type-B = 's one-line `qwen35.rs` change** — the **main-model +
 ...
 wrong for the final norm.** never touched `qwen35.rs` → persists to
...
overlaid commit `..HEAD` and genuine HEAD emit the *identical*
`fkk`-117 output → the overlay faithfully reproduces HEAD. `+OFFSET =
COHERENT` vs `+OFFSET = SCRAMBLED` is the adjacent flip — same commit as
...
**Fix:** revert both `load_final_norm_offset` sites to
`loader.load_vec(...)` (= pre- behavior) and delete the helper.
...
- **When the same symptom class has bitten before, TEST the prior claim first.**
 #48's relay named on day one; eight hypotheses later a bisect
...
- **One commit can carry two bugs; a partial revert leaves the quiet one.**
 touched both a `.cu` kernel (Type-A, loud — killed greedy) and a
 `.rs` loader (Type-B, quiet — greedy survived, only temp>0 broke).
...
- **A "fix" is a suspect.** and were both *fixes* that
 regressed even had a passing smoke. Length-dependent damage escapes
