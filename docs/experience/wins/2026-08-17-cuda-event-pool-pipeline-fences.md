...
> regression persisted because the event pool commit accidentally
> reverted 's fix that kept `all_reduce_sum` on the compute stream.
...
>. Measured TP=8 @100K: 70–80 tok/s (matching the 78.7 baseline).
...
```bash
# A/B: baseline = ^ (comm-stream, per-fence alloc),
# treatment = (comm-stream, event pool)
...
- Baseline: `^` (comm-stream, per-fence `new_event(None)`)
- Treatment: (comm-stream, `Arc<Mutex<Vec<CudaEvent>>>` pool)
...
|---|---:|---|
| pre-comm-stream | 78.7 @128K | baseline |
| comm-stream, per-fence alloc | 55–59 | −25–30% |
| + all-reduce → compute stream | 63 | partial recovery |
| + event pool, AR back on Comm | 48–64 | event pool alone insufficient |
| + one-shot enabled (--comm-backend auto) | 51–53 | one-shot slower than NCCL |
| **+ AR on Compute stream** | **70–80** | **recovered** |
...
which already did but accidentally reverted.
...
vs 75 tok/s with NCCL on the compute stream. The previous DSv4 measurement
 showed one-shot as wall-neutral, but Qwen3.5/3.6's 64-layer
