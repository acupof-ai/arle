...
| stochastic (temp 0.9) | 4* | 0.776 | 0.0142 | [0.762, 0.789] | .758 .770 .786 .788 |
| teacher 35B-A3B (ceiling) | — | ~0.82 | — | — | (prior anchor) |

<sub>*stochastic seed4 + the step25 trajectory matrix were still evaluating when this
entry landed; reverse-KL (the winner) is complete 5/5. The numbers above will not move the verdict.</sub>
...
**reverse-KL wins** — highest mean (0.792) AND tightest spread (sd 0.0045); its
mean-CI [0.788, 0.796] barely overlaps forward-KL's [0.775, 0.786]. **stochastic
is worst and noisiest** (0.776, sd 0.0142 — 3× reverse-KL's), i.e. temperature-0.9
rollout *adds variance without lifting the mean*. Recipe verdict: **reverse-KL
greedy** is the default to carry forward.
