...
required). Three failures blocked the first attempts, all on...
...
2. Phase-D reloaded the judge (29.5 GB) before the per-round LoRA sync, evicting
 ...
 round and any loop-local flag resets across calls. `judge_batch`
 bypassed the first fix and had to be routed through the same gate.
3. The engine-ready channel sent `err.to_string()` (outermost context only), so
 ...
 channel restored the chain; first reproduction after the fix
