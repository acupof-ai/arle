...
1. **Skip native MTP when DSpark is active**: `!dspark_on &&`
...
2. **NVFP4 vs FP8 E4M3 detection**: `is_nvfp4` checked only the
...
3. **W4AFP8 finalize kernel + SwiGLU cap re-applied**: the
 original fix commit was replaced by which kept only
...
4. **Loader shape validation**: rank check (`weight.shape.len() != 2`)
...
5. **Log message** (+): the spec-decode log now
