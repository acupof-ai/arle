...
> **Source of truth = code, not docs.** Rebuilt 2026-07-12 by re-scanning all
> **61** `.cu` under `crates/cuda-kernels/csrc/{attention,comm,deepep_sidecar,
> gemm,kv,kvcacheio,misc,moe,quant}/`, the FFI decls in
> `crates/cuda-kernels/src/ffi/`, and the **live callers in
> `crates/infer-cuda/src/`**. This rebuild vs the 2026-06-30 snapshot: **added
...
> ring KV prep), **added the four previously-missing dir sections**
> (`comm/`, `kvcacheio/`, `quant/`, `deepep_sidecar/`), **promoted to live**
...
- **61 `.cu`** across 9 dirs; `deepep_sidecar/` holds a `.cpp` sidecar binary,
  **0 `.cu`** → 61 kernel files in 8 kernel dirs.
...
| attention (FlashMLA) | `misc/arle_flashmla_decode_shim.cu`, `attention/arle_flashmla_decode_stubs.cu` | `arle_flashmla_sm90_sparse_decode_fwd` | `attention.rs::mla_attention`, `attention/flashmla.rs` | SM90 sparse MLA decode (vendored FlashMLA bridge) | bf16 / fp8-e4m3 kv |
| attention (FlashMLA) | `misc/arle_flashmla_shim.cu` | `arle_flashmla_sm90_sparse_prefill_fwd` | `attention.rs::mla_attention` | SM90 sparse MLA prefill | bf16 |
...
## attention/ — **MISPLACED**: DSv4 MLA + DSA + MHC ops living in `csrc/misc/`

Core DSv4 attention kernels sitting under `csrc/misc/`; belong in `csrc/attention/`.

| family | csrc file | FFI symbol | Rust caller | op shape | precision |
|---|---|---|---|---|---|
| attention (DSv4) | `misc/dsv4_attention.cu` | `dsv4_prepare_qk_cuda` / `…_start_pos_ptr_cuda` / `…_fused_batch_start_pos_cuda` | `attention.rs::mla_attention` | Q/K RoPE prep, d_qk 576 (512 NoPE+64 RoPE) | bf16 |
...
| attention (DSv4) | `misc/dsv4_attention.cu` | `dsv4_compressor_update_cuda` / `…_start_pos_ptr_cuda` / `…_batched_start_pos_ptr_cuda` | `attention.rs::compressor_forward` | compressor KV update | bf16 |
| attention (DSv4) | `misc/dsv4_attention.cu` | `dsv4_update_window_cache_cuda` / `…_start_pos_ptr_cuda` / `…_batched_ptr_cuda` | `attention.rs` (Route A window cache) | sliding window cache write | bf16 |
| attention (DSv4, Route A) | `misc/dsv4_attention.cu` | `dsv4_oproj_group_gather_cuda` / `dsv4_oproj_group_scatter_cuda` | `attention.rs::dsv4_oproj_group_{gather,scatter}` | o-proj group gather/scatter over compressor-state pool (`6a78a490d`) | bf16 |
| attention (DSv4) | `misc/dsv4_attention.cu` | `arle_dsv4_output_inverse_rope_cuda` / `…_start_pos_ptr_cuda` / `…_batched_ptr_cuda` / `…_batch_start_pos_cuda` | `attention.rs::mla_attention` (FlashMLA out path) | inverse-RoPE on MLA output | bf16 |
| attention (DSv4 DSA) | `misc/dsv4_dsa_official.cu` | `dsv4_deepseek_v4_topk_transform_cuda` | `attention.rs` (DSA select) | official DSA top-k transform | bf16 |
| attention (DSv4 DSA) | `misc/dsv4_dsa_official.cu` | `dsv4_dsa_build_select_meta_cuda` | `attention.rs` | DSA block-select metadata | i32 |
| attention (DSv4 DSA) | `misc/dsv4_dsa_official.cu` | `dsv4_dsa_fused_q_indexer_rope_hadamard_quant_cuda` | `attention.rs` | fused Q-indexer RoPE+Hadamard+quant | bf16→fp8 |
| attention (DSv4 DSA) | `misc/dsv4_dsa_official.cu` | `dsv4_dsa_fused_store_index_k_cache_cuda` / `…_batched_cuda` | `attention.rs` | fused index-K cache store | fp8-e4m3 |
| attention (DSv4 DSA) | `misc/dsv4_dsa_official.cu` | `dsv4_dsa_hadamard128_bf16_cuda` / `…_batched_cuda` | `attention.rs` | 128-dim Hadamard rotate | bf16 |
| attention (DSv4 DSA) | `misc/dsv4_dsa_official.cu` | `dsv4_dsa_fill_context_lens_positions_start_pos_cuda` | `attention.rs` | context-len/position fill | i32 |
| attention (DSv4 MHC) | `misc/dsv4_mhc.cu` | `dsv4_mhc_params_cuda` / `dsv4_mhc_params_pre_rms_norm_cuda` | `hc.rs::gen_mhc_params{,_into}` | multi-head-compressor sinkhorn params | bf16/f32 |
| attention (DSv4 MHC) | `misc/dsv4_mhc.cu` | `dsv4_mhc_expand_cuda` | `hc.rs::initial_stream_from_embeddings` | MHC stream expand | bf16 |
| attention (DSv4 MHC) | `misc/dsv4_mhc.cu` | `dsv4_mhc_pre_cuda` / `dsv4_mhc_pre_rms_norm_cuda` | `hc.rs::hc_pre` | MHC pre-projection | bf16 |
| attention (DSv4 MHC) | `misc/dsv4_mhc.cu` | `dsv4_mhc_post_cuda` | `hc.rs::hc_post` | MHC post-projection | bf16 |
| attention (DSv4 MHC) | `misc/dsv4_mhc.cu` | `dsv4_mhc_head_pre_cuda` | `hc.rs::head_hidden_from_stream` | MHC per-head hidden | bf16 |
| attention (DSv4 MTP) | `misc/dsv4_mhc.cu` | `dsv4_mtp_add_eproj_hproj_cuda` | `dsv4.rs` (MTP head) | MTP e-proj + h-proj add | bf16 |
| attention (DSv4 TP) | `misc/dsv4_tp_attention_repack.cu` | `dsv4_tp_q_repack_cuda` / `dsv4_tp_out_slice_cuda` | `attention.rs::mla_attention` (TP=8) | TP Q repack + output slice | bf16 |
...
## misc/ — sampling (incl. DSpark) + norm + elementwise + linear-attn

| family | csrc file | FFI symbol | Rust caller | op shape | precision |
|---|---|---|---|---|---|
| sampling | `misc/sampling.cu` | `argmax_cuda` / `argmax_batch_cuda` | `ops.rs::argmax{,_batch}` | greedy argmax over vocab | bf16/f32 logits |
| sampling (DSpark) | `misc/sampling.cu` | `dspark_draft_sample_cuda` | `qwen35/dspark.rs::dspark_draft_block` | draft-model filtered multinomial sample (`e22a41637`) | bf16 logits |
| sampling (DSpark) | `misc/sampling.cu` | `dspark_filter_probs_cuda` | `qwen35/dspark.rs::dspark_accept_commit_sampled` | device-side top-k/top-p prob filter | bf16→f32 |
| sampling (DSpark) | `misc/sampling.cu` | `dspark_chain_accept_cuda` | `qwen35/dspark.rs::dspark_accept_commit_sampled` | chain-rejection accept + bonus resample | f32 |
...
| norm | `misc/norm.cu` | `rms_norm_offset_cuda` / `rms_norm_batched_offset_cuda` | `qwen35.rs::rms_norm_offset{,_vec}` | offset RMSNorm | bf16 |
| norm | `misc/norm.cu` | `rms_norm_gated_cuda` | `qwen35.rs::linear_attention` | gated RMSNorm (GDR) | bf16 |
| elementwise | `misc/elementwise_basic.cu` | `add_cuda` / `add_scaled_row_cuda` | `ops.rs::add_batch`, `attention.rs` | elementwise residual/scaled add | bf16 |
...
| dtype | `misc/arle_dtype_convert.cu` | `arle_bf16_to_f32_cuda` | `attention.rs` / `ops.rs` | bf16→f32 cast | bf16→f32 |
...
| linear-attn (Qwen3.5) | `misc/gated_delta_rule.cu` | `gdr_fq_prep_cuda` (+ fixed-point `gdr_fq_{kkt,fwd,cumsum}_cuda`) | `qwen35.rs::linear_attention` | GDR fixed-point-quant solve prep | bf16 |
| linear-attn (Qwen3.5) | `misc/gdr_decode_batch.cu` | `gdr_decode_batch_cuda` | `qwen35.rs::linear_attention` | batched GDR decode | bf16 |
| conv (Qwen3.5) | `misc/conv1d.cu` | `conv1d_prefill_cuda` | `qwen35.rs::linear_attention` | short-conv1d over prefill | bf16 |
| conv (Qwen3.5) | `misc/conv1d_decode_batch.cu` | `conv1d_decode_batch_cuda` | `qwen35.rs::linear_attention` | batched conv1d decode | bf16 |
...
## kvcacheio/ — KV-tier page transport (present, UNWIRED from infer-cuda)

| family | csrc file | FFI symbol | status |
|---|---|---|---|
| kv-tier | `kvcacheio/transfer.cu` | `transfer_kv_pages_layer_table_cuda` | wrapper `[ck paged_kv.rs::transfer_layer_table_pair]` exists but **not reached from `infer-cuda`** — dormant KV-tier transport |
...
|---|---|---|---|
| kv migrate | `kv/kv_cache_to_paged.cu`, `kv/scatter_kv.cu` | `kv_cache_to_paged_*_cuda`, `scatter_write_kv_cuda` | 0 infer-cuda caller |
| kv append | `kv/paged_kv_append.cu`, `kv/paged_kv_metadata.cu` | `paged_kv_append_cuda`, `paged_kv_append_{new_page,last_token}_indices_cuda` | internal to `[ck tilelang.rs]` only |
...
| FlashMLA CSA prep | `misc/arle_flashmla_csa_prep.cu` | (no `extern "C"` in-file; helper TU) | linked into shim only |
...
| Marlin W4/W4A8 GEMM | `gemm/marlin_kernel.cu`, `marlin_w4_fp8_kernel.cu`, `marlin_w4a8_kernel.cu` | `marlin_gemm_cuda`, `gemm_w4_fp8_marlin_cuda`, `gemm_w4a8_marlin_cuda` | repack/preprocess (`marlin_repack.cu`, `marlin_int4_fp8_preprocess.cu`) live; the GEMM kernels 0 caller |
| Activation-quant + weight-gemv | `gemm/w4_fp8_activation_quant.cu`, `gemm/w4a8_activation_quant.cu`, `gemm/turboquant_weight_gemv.cu` | `quantize_bf16_rows_to_{fp8_e4m3,int8}_cuda`, `turboquant_weight_{gemv,dequant}_cuda` | 0 caller (paired with the unwired Marlin/TQ GEMM lanes) |
...
| MHC bench | `misc/dsv4_mhc.cu` | `dsv4_mhc_params_bench_cuda`, `dsv4_mhc_pre_rms_norm_bench_cuda` | bench-only, 0 runtime caller |
| DSA/DSv4 misc | `misc/dsv4_dsa_official.cu` | `dsv4_deepseek_v4_topk_transform` unused arms | secondary DSA arms unused; core family live above |
...
| family dir | verdict |
|---|---|
| `gemm/` | **well-organized** — all live GEMM/GEMV/DeepGEMM/Marlin-repack/quant-linear ops here; still carries a long tail of unwired Marlin/TQ-GEMM + activation-quant + GGUF variants to prune |
...
| `kv/`, `kvcacheio/`, `quant/` | **dormant** — internally coherent but the whole quantized-KV / KV-tier / TurboQuant subsystem is unwired in the rewrite (TileLang kv8 owns the live KV path) |
| `attention/` | **partly misplaced + newly-live** — live paged-prep + hd256 + FA3/FlashMLA shim + DSpark ring here, **but** the live DSv4 MLA/DSA/MHC kernels live in `misc/`; the DSv4-specific FP8-KV-pack + FlashMLA-index `.cu` here stay unwired (vendored builders won) |
| `misc/` | **MISPLACED dumping ground** — holds **core** ops: `norm.cu`, `sampling.cu` (incl. live DSpark chain-rejection), `elementwise_basic.cu`, `arle_dtype_convert.cu`, the entire **live DSv4 attention** (`dsv4_attention.cu`, `dsv4_dsa_official.cu`, `dsv4_mhc.cu`, `dsv4_tp_attention_repack.cu`), Qwen3.5 linear-attn (`gated_delta_rule.cu`, `gdr_decode_batch.cu`, `conv1d*.cu`), and the FlashMLA/FA3 shims (`arle_flashmla_*_shim.cu`). Recommend: `norm.cu`→`norm/`, `sampling.cu`→`sampling/`, `elementwise_basic.cu`+`arle_dtype_convert.cu`→`elementwise/`, `dsv4_{attention,dsa_official,mhc,tp_attention_repack}.cu`→`attention/`, `gated_delta_rule.cu`+`gdr_decode_batch.cu`+`conv1d*.cu`→`recurrent/`, `arle_flashmla_*_shim.cu`→`attention/` (matching the `src/ffi/{norm,sampling,elementwise,recurrent}.rs` split that already exists). |
