...
| Qwen3.8 | Supported (CUDA) | `unsloth/Qwen3.8-27B-NVFP4` — mixed-precision: NVFP4 MLP (W4AFP8) + FP8 per-channel attention, same `qwen3_5` architecture as Qwen3.5/3.6. Loads in 6.5s on H20, 20.69 GiB peak RSS, FP8 KV cache. c=1 decode **66.6 tok/s**, 12–17% ahead of Qwen3.6-27B-FP8 on the same box at both 8-token and 33K context. **Not yet serving-ready:** c≥2 loses 5× to FP8 and the 32K run does not complete — one dequant arm firing at `M >= 2` re-materialised all 11.56 G FP8 params per step ([errors](experience/errors/2026-08-19-fp8-dequant-arm-shadows-decode.md), fix landed, re-measure pending). Spec decode is net-negative until that lands. ([wins](experience/wins/2026-08-18-qwen38-27b-nvfp4-inference.md), [rows](baselines.md)) |
...
| Weights — NVFP4 (Fp4E2M1Group) | production (CUDA) | compressed-tensors `U8` packed FP4 + FP8 per-group scales + F32 global/input scales; auto-detect. Hand-written GEMV for decode, dequant→BF16→cuBLAS for prefill. |
