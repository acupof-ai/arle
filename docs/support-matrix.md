...
| --- | --- | --- | --- |
| Metal (`infer-metal`) | **Verified** | Real MLX Qwen3.5/3.6 forward, **bit-identical greedy parity** vs legacy across 4 configs (Qwen3.5-0.8B single-token / full 16-tok / chunked prefill, and canonical Qwen3.6-35B-A3B-4bit MoE). Cross-step decode pipeline recovered the c=1 perf regression. Target-only Metal is single-flight. With a loaded DFlash/NextN runtime, the executor advertises `INFER_METAL_DFLASH_MAX_ROWS` (default 16) and supports DFlash-specific multi-row and mixed scheduling. | Full packed batched-decode parity with CUDA; FP8/4-bit Metal MoE quant swap points. |
| CUDA (`infer-cuda`) | **Verified (serve: prefill + decode)** | Qwen3.5/3.6/3.8 hybrid (§1); DSv4-Flash **TP=8/EP=8** (MLA+CSA/HCA+FP8 DeepGEMM MoE, FlashMLA, DeepEP/allreduce) serves in-process multi-rank (`63d814a4`); per-layer RoPE theta fix (`fa355315`) → needle exact at 32K; KV-precision parity gate re-ported + FlashMLA decode licensed (#58, 2026-06-10); INT8/FP8 paged quant-KV is **Qwen3.5-family only** — `loaded.rs:1021` rejects any non-BF16 `--kv-cache-dtype` for DSv4 at engine construction (#68 T3 wired one model kind at a time); DSv4 B>1 decode now takes the batched lane by default, and MTP D2/T2 verifies with chain-shaped rows (`draft_rows=2`, `verify_rows=3`, not full-tree 7) when `--spec-type mtp --mtp-draft-tokens 2 --mtp-draft-topk 2` is enabled. | DP-attn (#89) and lm_head vocab-parallel shard (#99) closed 2026-08-13/17 — the shard was implemented, never licensed, and deleted; DP-attn washed at B=1, long-ctx multi-shape tracked under CP (#207/#226). (#56/#57 closed.) |
| Vulkan (`infer-vulkan`) | **Experimental (bring-up)** | Qwen3.5 27B dense (`Qwen3.6-27B-Q8_0`) GGUF forward is now **coherent** on Radeon 8060S (Strix Halo, gfx1151) — three HF→GGUF convention bugs fixed (plain-`w` RMSNorm, gated-delta decay sign, partial rotary `rope.dimension_count=64`); the 35B-A3B MoE FFN now runs coherent too (`79598456`). A record-many/submit-once recorder + real `vk::PipelineCache` + arena take the dense forward **5.0→3.0 s/tok** (perf Steps 1–4). Device residency upload, per-row dequant, GGUF→`Qwen35Config` mapper, and a GGUF-embedded BPE tokenizer landed. | Still ~0.33 tok/s vs llama.cpp's 7.2 tg reference bar for the same shape (wins); a `vulkan` serve builder is registered and selectable via `arle serve --backend vulkan`, but numeric forward validation on the serve path is still pending; perf-parity is the bring-up goal (plan, #71). |
...
vanilla Qwen3-MoE (`Qwen3MoeForCausalLM`) now **fails closed at engine load**
on CUDA — it never runs the forward — with "use --backend metal"
(`loaded.rs:1006`); the earlier numerically-wrong-forward investigation is
tracked in [#120](https://github.com/acupof-ai/arle/issues/120).
Qwen3.5/3.6 + DeepSeek-V4-Flash (TP=8/EP=8) on CUDA (prefill + decode verified,
...
production-verified. On Metal, DeepSeek-OCR (DeepEncoder + MXFP8 MoE decoder, experimental) is the VLM bring-up.
CUDA/Vulkan VLM forward paths still fail closed.
...
pod verification (#85 landed verify pending-remote; DSv4 reuses via
...
| --- | --- | --- |
| Qwen3 dense (`model_type=qwen3`) | Not supported on CUDA | Removed 2026-08-22; `LoadedInferenceEngine` fails closed with "Qwen3 dense is not supported on CUDA; use a Qwen3.5-family checkpoint". Metal and Vulkan unchanged. |
...
| Qwen3.6 / Qwen3.5-MoE | Supported (Metal + CUDA) | `mlx-community/Qwen3.6-35B-A3B-4bit` is the **canonical Metal production model** (globally unified 2026-05-07) — every Metal serve/bench/test defaults to it. **CUDA serving landed**: FP8 DeepGEMM MoE + batched paged decode (Qwen3.6-27B-FP8 1×H20 tok/s scales c=1→8: 21→26, [wins](experience/wins/2026-06-29-cuda-qwen36-paged-batched-decode.md)); vanilla Qwen3-MoE fails closed at CUDA engine load ("use --backend metal", `loaded.rs:1006`) — it no longer runs the forward; tracked in [#120](https://github.com/acupof-ai/arle/issues/120) (wins). Qwen3.5-122B-A10B serves at TP4 via GQA KV-head replication (TP > num_kv_heads, all 4 worker engines ready) — numerical-completion gate pending a clean re-run ([wins](experience/wins/2026-06-29-cuda-gqa-replication-122b-tp4.md)). |
| Qwen3.8 | Supported (CUDA) | `unsloth/Qwen3.8-27B-NVFP4` — mixed-precision, same `qwen3_5` architecture as Qwen3.5/3.6. Only 54% of params are 4-bit (7.49 GB U8 MLP); attention, lm_head and layers 56-63 are per-channel FP8 (11.56 GB), so the checkpoint is 23.42 GB against the FP8 model's 30.87 GB — 24% fewer bytes, not half. Marlin at decode, DeepGEMM's native FP8 MMA at prefill (the operand is derived from Marlin's layout into scratch, so nothing is resident twice). Against Qwen3.6-27B-FP8 on one H20, same binary, `--max-running-requests 16` (32K long-agent, p50 of 32 requests): ITL advantage **+29.0% / +28.9% / +22.2% / +5.3%** at c=1/4/8/16, measured 2026-08-22 ([rows](baselines.md), NVFP4 anchor "Long-agent 32K"). This supersedes the 2026-08-20 figures (+21.3% / +20.7% / +13.2% / +5.5%): the NVFP4 checkpoint did not change between the runs — the FP8 comparison baseline improved 14.6% through concurrent Marlin and Markov-head work, so the measured delta moved without the treatment changing. The 2026-08-20 end-to-end deltas (**+5.0% / +15.3% / +9.1% / +1.8%**) and the 8-token synthetic-prompt decode deltas (**+37.9% / +41.4% / +39.5% / +36.7% / +13.7%** at c=1/2/4/8/16) have not been remeasured by any later run; quote them only as 2026-08-20 numbers ([wins](experience/wins/2026-08-20-nvfp4-widen-to-e4m3-deepgemm-prefill.md)). Resident 22.36 GB against FP8's 29.36 GB, KV pool 1,779,114 against 1,582,506. Needle 3/3 exact at 512/4096/16384/32768. |
| DeepSeek V4 | Serving (CUDA 8×H20 TP=8/EP=8) | DSv4-Flash serves via `arle serve --backend cuda` in-process multi-rank: FlashMLA + DSA/CSA/HCA hybrid attention, FP8 KV, DeepGEMM FP8 MoE, DeepEP/allreduce transports. Needle-exact to 32K after the per-layer RoPE theta fix (`fa355315`). Current MTP path is explicit (`--spec-type mtp`): D2/T2/topk=2 uses the B>1 batched spec lane and keeps verifier cost chain-shaped (`verify_rows=3`; wins). DP-attn (#89) and lm_head shard (#99) are closed — the shard was implemented, never licensed, and deleted; DP-attn washed at B=1, long-ctx tracked under CP (#207/#226). Phase-0 debt (#56/#57) closed; umbrella #55. **GLM-5.2** (`glm_moe_dsa`, DeepSeek-V3.2-DSA family, 256 experts) rides this same DSv4 CUDA path via an adapter — forward tranches wired, but verification is [pending-remote](experience/wins/2026-06-19-glm52-tranche-d-forward-pending-remote.md), not production-verified. `crates/deepseek-spec` now carries V4 + GLM-5.2; DSv4 scratch pretrain stays retired (2026-05-18 OPD-only pivot). |
...
| Gemma | Not carried | Removed 2026-08-24; reintroduction requires a new explicit project. |
| Mistral / Mixtral / Phi | Planned | Not yet supported. |

**Next-model roadmap priority** (cross-surface ranking: [docs/plans/2026-08-24-roadmap.md](plans/2026-08-24-roadmap.md)):

1. **DeepSeek V4 (DS4)** — serves at TP=8/EP=8 on CUDA; GLM-5.2 (`glm_moe_dsa`) rides the same path (verification pending-remote). Long-ctx residual is the remaining runtime lever; DP-attn (#89) and lm_head shard (#99) were measured and killed (2026-08-13/17).
...
locations, and test-harness semantics. The summary table
...
| INT8 KV cache (Metal + CUDA) | production (Metal default; CUDA production) | Metal `--kv-cache-dtype auto` resolves to `int8` and stores full-attention K/V as MLX affine 8-bit packed triples; `bf16` remains the explicit fallback. CUDA `--kv-cache-dtype int8` uses per-(token, head) K and V scales with decode on `paged_attention_quantized_fa3.cu`; +57–113% throughput vs BF16 on A100 (`wins/2026-05-26-bench-int8-vs-bf16-kv-a100`, dense-Qwen3 era). |
| FP8 E4M3 KV cache (CUDA) | opt-in | `--kv-cache-dtype fp8`; per-(token, head) K and V scales, same decode kernel as INT8. |
| TurboQuant KV TQ4 (CUDA) | deferred | `--kv-cache-dtype tq4`; accepted by the CLI, bails loud at engine construction (`infer-cuda/src/executor.rs:108`). |
| Weights — W4A16 / W8A16 | production (CUDA) | W4A16 is GEMV-only — the new stack has **no Marlin or dequant arm at any M** (`ops/quant_linear.rs:619`); `marlin_w4a16_gemm_cuda` is declared in FFI but has no caller. W8A16 has a real Marlin prefill arm with GPTQ repack. safetensors auto-detect. |
| Weights — W2A16 | not implemented | `WeightFormat::W2A16` is an enum variant only; there is no `QuantFormat`, loader arm, or kernel. |
| Weights — GGUF Q4/Q5/Q6_K (+ Q8_0, Q2_K) | experimental (Vulkan & HIP) | GGUF is **not** served on CUDA or Metal (both new-stack loaders require safetensors and neither depends on `infer-gguf`). It ships on the experimental backends: Vulkan CPU-dequants Q2_K/Q4_K/Q5_K/Q6_K/Q8_0 (`infer-vulkan/loader.rs`), HIP keeps Q4_K/Q5_K/Q6_K quantized via the `iq2_mmvq`/`quantized_gemv` kernels. `Q3_K` has no host launcher. `.gguf` auto-detect on those backends. |
| Weights — DSv4 FP8/FP4 block-scaled | production (CUDA, TP=8/EP=8) | `Dsv4Fp8BlockScaled` / `Dsv4Fp4BlockScaled` are dispatched on the serving path (`dsv4/load.rs:929`, attention + MoE W4AFP8 grouped GEMM); see the DeepSeek V4 row above for the verified TP=8/EP=8 FP8 configuration. |
...
| Weights — FP8 per-channel (Fp8BlockScaled{1,K}) | production (CUDA) | compressed-tensors `F8_E4M3` + BF16 [N,1] weight_scale; auto-detect as block-scaled with block_m=1. Marlin below the DeepGEMM prefill floor; above it the E4M3 bytes are un-permuted out of Marlin's tiles into scratch — byte-identical to the source — and contracted natively, with the channel scale applied to the output columns afterwards. |
...
- **Rewrite-stack CUDA `--kv-cache-dtype` dispatch landed (#68, 2026-06-12).**
 Seam-level INT8/FP8 paged quant-KV on the CUDA path is wired end-to-end
 (row-quantize / split-KV quantized decode) —
...
| --- | --- | --- |
| Slot-sticky multi-turn KV reuse | Supported (CUDA), Beta (Metal) | Prior-turn KV stays in slot for the next turn so only new user tokens prefill. CUDA is the primary path; Metal Qwen3.5/3.6 restores the shared prefix from boundary snapshots — verified across 12 agent turns 2026-09-02 (turns 2–12 TTFT 180 ms vs 2.01 s before the fix, restored output byte-identical to cold; [wins](experience/wins/2026-09-02-metal-prefix-restore-survives-turns.md)). |
| Radix-backed prefix cache (T0 GPU) | Supported (CUDA Qwen3.5/3.6 via sidecar), Beta (Metal; snapshot-clamped, multi-turn verified 2026-09-02) | Page-aligned full-block radix (`RadixCache`) with retain/release refcounts and LRU evict. Qwen3.5/3.6 hybrid re-enabled via the recurrent sidecar snapshot protocol (#85; pod verify pending-remote); DSv4 stays off the page radix by design — it reuses via the position-0 whole-slot store (§4b). No tail-page CoW (no n>1 consumer, #86 closed as backlog). |
| T1 DRAM tier (host store) | **Default-on (all CUDA arms), verified on H20 pod** | One `KvTierStore` (`kv-native-sys`) per arm. Qwen3.5/3.6: whole-slot G3 capacity spill, stored as 16 MiB chunked blobs (same path as DSv4). The page-granular `KvPageTier` path is stubbed for Qwen3.5/3.6 by design — recurrent sidecars carry the restorable state, full-attn KV pages stay device-resident under radix reuse; a prefix-cache eviction under VRAM pressure loses full-attn pages (recompute on next hit). DSv4: whole-slot spill stored as 16 MiB chunks under a per-key manifest. Budgeted by `--kv-dram` (bytes / `%` of MemAvailable / `0`=off; default `50%`) — a DEPLOYMENT-TOTAL cap split across TP ranks at engine build. Read-hit telemetry (`kv_tier_read_hits`) and L3→L2 promote-on-read are wired on all arms ([wins](experience/wins/2026-08-17-kv-tier-read-hits-and-promote-on-read.md)). [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md). |
| T2 NVMe disk spill | **Opt-in (DSv4 TP=4 multiproc verified on H20 pod)** | `--kv-disk [DIR]` + `--kv-disk-limit` (bytes / `%` of free disk; default 50% of free disk at the root) ride `EngineLoadConfig` (`kv_ssd_root`/`kv_disk_limit`). The L3 cap is DEPLOYMENT-TOTAL, split across TP ranks at engine build; every rank attaches the tier inside the engine constructor — single-proc and multiproc TP workers alike; each process namespaces its own `KvMmapStore` (sparse mmap page-slot store) under the root. Per-arm granularity matches T1 (Qwen3.5/3.6 whole-slot; DSv4 16 MiB chunks). Metal attaches at construction; non-consuming backends fail closed. [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md), [wins](experience/wins/2026-07-02-dsv4-kvssd-multiproc-attach.md). |
| T3 cluster-shared backend | Stub only | **NIXL transport remains stub-only** (no `nixl-sys` crate or real link yet); the rewrite has no KV export/import surface. P/D-disagg backlog in #87. |
| DSv4 position-0 prefix cache | **Default-on, rides THE tier store — pod-verified through NVMe** | Token index (`infer_kvspace::PrefixPoolIndex`, implemented for the CUDA `Dsv4PrefixStatePool`; the old private `PrefixIndex` store was deleted in `af100d0fc`) over snapshot blobs stored in the shared `KvTierStore` (`kv-native-sys`) under `NS_PREFIX*` key namespaces (16 MiB chunks) — budgeted by the ONE DRAM budget, spills to NVMe when `--kv-disk` is set. LRU under store pressure (prefixes yield, parked slots never touched). The private BTreeMap cache and its `ARLE_DSV4_PREFIX_CACHE_BYTES` knob are deleted. TP=4 verified: 3375-tok prompt cold 26.6s → restored **0.47s** from disk, hit_rate 1.0, zero restore errors ([wins](experience/wins/2026-07-02-kv-unified-tier-flags-pod-verify.md)). Blob accounting attributed and bounded (supersede-in-place verified via capture logs; distinct suffixes bounded by the per-rank cap + coldest eviction); store write 1.20 GiB/s / warm read 2.22 GiB/s vs 197 MB/s device QD1 ([wins](experience/wins/2026-07-02-kv-tier-round4-close.md)). |
| Swap-style preemption | **Shipped with the tier (tier-gated)** | The plan-repair loop in `fit_plan_to_kv_pages` (`crates/infer-core/src/planner.rs`; the old `retract_decode_to_fit`, renamed in `2f96eb633`) seals the victim's prompt blocks into the radix and demotes exactly those pages; re-admission promotes instead of re-prefilling (decode is still recomputed). Without a tier store the recompute path is byte-for-byte unchanged. DSv4 whole-slot variant tracked under #84/#85 Route B. |
...
| Structured-output grammar (xgrammar FFI) | Scaffold (Phase 1) | `crates/xgrammar-sys` Rust safe wrapper over upstream `mlc-ai/xgrammar` v0.1.34 (codex's #26 WIP, FFI substrate landed; default build = stub; the C++ shim is built through the `grammar` feature, which forwards `cli/grammar` → `infer-api/grammar` → `infer-server/grammar` → `xgrammar-sys/real` (`cc` + pinned upstream checkout). Build the leaf directly with `-p xgrammar-sys --features real`; there is no root-level `real` feature). No HTTP, scheduler, sampler, or GPU sampling integration yet. |

## 5a. Training Surface Matrix

> **2026-05-18 pivot — OPD only.** Scratch pretrain, SFT, GRPO, and
> multi-turn RL surfaces were retired in that pivot.
...
Two objective families ride one substrate (teacher/rollout on the in-process
infer engine via `train::teacher_infer`, student
