...
| Qwen3.8 | Supported (CUDA) | `unsloth/Qwen3.8-27B-NVFP4` — mixed-precision, same `qwen3_5` architecture as Qwen3.5/3.6. Only 54% of params are 4-bit (7.49 GB U8 MLP); attention, lm_head and layers 56-63 are per-channel FP8 (11.56 GB), so the checkpoint is 23.42 GB against the FP8 model's 30.87 GB — 24% fewer bytes, not half. Marlin at decode, DeepGEMM's native FP8 MMA at prefill (the operand is derived from Marlin's layout into scratch, so nothing is resident twice). Against Qwen3.6-27B-FP8 on one H20, same binary, `--max-running-requests 16`: 32K long-agent ITL **+21.3% / +20.7% / +13.2% / +5.5%** at c=1/4/8/16, end-to-end **+5.0% / +15.3% / +9.1% / +1.8%** (positive on both metrics at every point); 8-token decode **+37.9% / +41.4% / +39.5% / +36.7% / +13.7%** at c=1/2/4/8/16. Resident 22.36 GB against FP8's 29.36 GB, KV pool 1,779,114 against 1,582,506. Needle 3/3 exact at 512/4096/16384/32768. ([wins](experience/wins/2026-08-20-nvfp4-widen-to-e4m3-deepgemm-prefill.md), [rows](baselines.md)) |
