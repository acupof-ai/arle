...
| Qwen3.8 | Supported (CUDA) | `unsloth/Qwen3.8-27B-NVFP4` — mixed-precision, same `qwen3_5` architecture as Qwen3.5/3.6. Only 54% of params are 4-bit (7.49 GB U8 MLP); attention, lm_head and layers 56-63 are per-channel FP8 (11.56 GB), so the checkpoint is 23.42 GB against the FP8 model's 30.87 GB — 24% fewer bytes, not half. Every quantised GEMM runs on Marlin (`fp4_route` / `fp8_route`, both dispatch lanes). Against Qwen3.6-27B-FP8 on one H20, same binary: 8-token decode **+37.9% / +41.4% / +39.5% / +36.7% / +13.7%** at c=1/2/4/8/16; 32K long-agent **+21.3% / +18.4% / +11.5%** ITL at c=1/4/8 and **+12.7% / +7.0%** end-to-end output from c=4 (c=1 trails 33.9% end-to-end — that workload is 154:1 prefill-to-decode and Marlin's dequant→BF16 MMA is behind DeepGEMM's native FP8 MMA at prefill). Needle 3/3 exact at 512/4096/16384/32768. ([wins](experience/wins/2026-08-20-marlin-source-freed-18gb.md), [rows](baselines.md)) |
