...
| Capability | Status | Notes |
| --- | --- | --- |
| Metal DFlash (Qwen3.5) | **Shipped for compatible models** | `--draft-model` selects an explicit compatible draft; CUDA-oriented `--spec-type` and `--mtp-draft-model` are rejected on Metal. |
...
| Metal DFlash (Qwen3.5-MoE) | **Shipped for compatible models** | Explicit compatible drafts route through the live Metal DFlash runtime; model/checkpoint compatibility still fails closed. |
