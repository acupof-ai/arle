...
a matched A/B run says so. Project framing:
[index.md §Positioning](index.md#positioning-fact).
...
CUDA VLM forward paths still fail closed.
...
below predates the rewrite; verify against §0.
...
| --- | --- | --- |
| Qwen3 dense (`model_type=qwen3`) | Not supported on CUDA | Removed 2026-08-22; `LoadedInferenceEngine` fails closed with "Qwen3 dense is not supported on CUDA; use a Qwen3.5-family checkpoint". Metal unchanged. |
...
| INT8 KV cache (Metal + CUDA) | production (Metal default; CUDA production) | Metal `--kv-cache-dtype auto` resolves to `int8` and stores full-attention K/V as MLX affine 8-bit packed triples; `bf16` remains the explicit fallback. CUDA `--kv-cache-dtype int8` uses per-(token, head) K and V scales with decode on `paged_attention_quantized_fa3.cu`; +57–113% throughput vs BF16 on A100 (dense-Qwen3 era). |
...
[arle-opd](https://github.com/acupof-ai/arle-opd) repository and consumes this
repo's crates. This runtime serves as its teacher through `/v1/raw_logits` and
`--lora-adapters`.
