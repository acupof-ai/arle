...
| T1 DRAM tier (host store) | **Default-on (all CUDA arms), verified on H20 pod** | One `CudaKvTierStore` per arm. Qwen3-dense: page-granular radix demote/promote. Qwen3.5/3.6: whole-slot G3 capacity spill (one slot image per store page). DSv4: whole-slot spill stored as 16 MiB chunks under a per-key manifest. 4 GiB floor, profiled from MemAvailable via `--dram-fraction 0.5`; `--kv-t1-budget-bytes 0` disables. [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md). |
| T2 NVMe disk spill | **Opt-in (Qwen3-dense verified on H20 pod)** | `--kv-ssd-path` + `--kv-ssd-max-bytes` (machine-derived from free disk via `--ssd-fraction 0.5` when unset) ride `EngineLoadConfig` (`kv_ssd_root`/`kv_ssd_max_bytes`), so every rank attaches the tier inside the engine constructor — single-proc and multiproc TP workers alike; each process namespaces its own `KvMmapStore` (sparse mmap page-slot store) under the root. Per-arm granularity matches T1 (dense page-granular batch-H2D Manifest V2; Qwen3.5/3.6 whole-slot; DSv4 16 MiB chunks). Metal attaches at construction; non-consuming backends fail closed. [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md). |
