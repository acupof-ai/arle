...
([wins](experience/wins/2026-06-29-qwen3-moe-cuda-servable.md)).
...
| CUDA DSpark block-drafter (Qwen3.6 + DSv4-Flash) | **Shipped** | `arle serve --backend cuda --spec-type dspark --mtp-draft-model <DSpark/DFlash dir>` loads the external 3-stage block drafter and runs draft→verify→accept in one batched target forward. Qwen3.6-27B + DSv4-Flash both supported. |
| DSpark train sidecar | **Shipped (opt-in)** | `--dspark-train` (requires `--spec-type dspark`, CUDA-only): spawns a background thread that drains the experience buffer the hot path populates and runs acceptance-weighted policy gradient + L1 probability matching on the Markov head, hot-swapping updated weights back into the running engine. E2E verified on H20 ([wins](experience/wins/2026-07-20-dspark-train-sidecar-e2e-verified.md)). No-op without `--spec-type dspark` or on non-CUDA backends. |
...
| DSpark train sidecar (`--dspark-train`) | **Shipped (opt-in)** | In-process acceptance-weighted trainer that runs alongside `arle serve --spec-type dspark`. Drains the experience buffer the hot path populates and updates the Markov head (PG + L1 probability matching), hot-swapping weights back into the running engine. CUDA-only. See §4a for the command and [`wins`](experience/wins/2026-07-20-dspark-train-sidecar-e2e-verified.md). |
