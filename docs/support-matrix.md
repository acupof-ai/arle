...
| T1 DRAM tier (host store) | **Default-on (CUDA Qwen3-dense + DSv4), verified on H20 pod** | Qwen3-dense: page-granular radix demote/promote. Qwen3.5/3.6: whole-slot G3 capacity spill. DSv4: per-layer FlashMLA page-tier via `CudaKvTierStore`. Whole-slot swap as capacity fallback. 4 GiB floor, profile from MemAvailable via `--dram-fraction 0.5`. `--kv-t1-budget-bytes 0` disables. [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md). |
| T2 NVMe disk spill | **Opt-in, verified on H20 pod (Qwen3-dense, DSv4 FlashMLA page-tier (per-layer, paged pool))** | `--kv-ssd-path` + `--kv-ssd-max-bytes` (machine-derived from free disk via `--ssd-fraction 0.5`). `KvMmapStore` sparse mmap page-slot store. Qwen3-dense: page-granular, batch H2D, Manifest V2. DSv4 page-tier: per-layer FlashMLA page demote/promote via KvMmapStore. DSv4/Qwen3.5 whole-slot images also route through the same store, but online use is gated by measured wall-clock vs recompute; the old fixed-upper-bound slot write path was being replaced by variable-length records. Non-consuming backends fail closed. [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md). |
...
| DSv4 position-0 prefix cache | **Default-on (host DRAM BTreeMap)** | `Dsv4PrefixCache` maps full prompt → whole-slot image. LRU over `ARLE_DSV4_PREFIX_CACHE_BYTES` (default 4 GiB). Pod-verified correct + 11.7× prefill speedup. Not yet migrated to KvMmapStore (stays in DRAM). |
