...
| T1 DRAM tier (host store) | **Default-on (CUDA Qwen3-dense), verified on H20 pod** | 4 GiB floor, profile from MemAvailable via `--dram-fraction 0.5`. `--kv-t1-budget-bytes 0` disables. Demote-on-evict / promote-on-match; LRU rotation when full. [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md). |
| T2 NVMe disk spill | **Opt-in, verified on H20 pod** | `--kv-ssd-path` + `--kv-ssd-max-bytes` (machine-derived from free disk via `--ssd-fraction 0.5`). `KvMmapStore` sparse mmap page-slot store (one file per namespace, 0% pre-alloc, memcpy write, zero-copy `&[u8]` read). Manifest V2 (`key slot_idx slot_bytes`) for durable restarts. Promoted pages get batch H2D dispatch (N→1 `cuMemcpyHtoDAsync` calls). Non-consuming backends fail closed. [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md). |
