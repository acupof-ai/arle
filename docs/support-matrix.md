...
| T1 DRAM tier (host store) | **Default-on (CUDA Qwen3-dense + DSv4), verified on H20 pod** | Qwen3-dense: page-granular radix demote/promote. Qwen3.5/3.6: whole-slot G3 capacity spill. DSv4: whole-slot swap via `CudaKvTierStore` (`slot_tier`). 4 GiB floor, profile from MemAvailable via `--dram-fraction 0.5`. `--kv-t1-budget-bytes 0` disables. [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md). |
| T2 NVMe disk spill | **Opt-in, verified on H20 pod (Qwen3-dense, DSv4 pending-remote)** | `--kv-ssd-path` + `--kv-ssd-max-bytes` (machine-derived from free disk via `--ssd-fraction 0.5`). `KvMmapStore` sparse mmap page-slot store. Qwen3-dense: page-granular, batch H2D, Manifest V2. DSv4: whole-slot image serialization + padding to fixed page size. Non-consuming backends fail closed. [wins](experience/wins/2026-06-30-kv-mmap-tier-e2e.md). |
