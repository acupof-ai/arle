...
mod tests {
    ...
    #[test]
    fn layer_schedule_matches_compress_ratios() {
        use deepseek_spec::v4::DeepSeekV4AttentionMode::*;
        let cfg = small_config(); // ratios [0, 4, 32], num_hash_layers 1
        let plans = layer_schedule(&cfg);
        assert_eq!(plans.len(), 3);
        assert_eq!(plans[0].mode, SlidingWindow);
        assert!(!plans[0].has_compressor && !plans[0].has_indexer);
        assert!(plans[0].hash_routing);
        assert_eq!(plans[1].mode, CompressedSparse);
        assert!(plans[1].has_compressor && plans[1].has_indexer);
        assert!(!plans[1].hash_routing);
        assert_eq!(plans[2].mode, HybridCompressed);
        assert!(plans[2].has_compressor && !plans[2].has_indexer);
    }

    #[test]
    fn gemv_dispatch_table() {
        assert_eq!(
            gemv_kind(Residency::KeepKQuant(KQuant::Q4K)),
            Some(GemvKind::Q4K)
        );
        assert_eq!(
            gemv_kind(Residency::KeepKQuant(KQuant::Q5K)),
            Some(GemvKind::Q5K)
        );
        assert_eq!(
            gemv_kind(Residency::KeepKQuant(KQuant::Q6K)),
            Some(GemvKind::Q6K)
        );
        assert_eq!(gemv_kind(Residency::KeepIq2Xxs), Some(GemvKind::MmvqIq2Xxs));
        assert_eq!(gemv_kind(Residency::KeepQ2K), Some(GemvKind::MmvqQ2K));
        // The fail-loud case: no bf16/f32 GEMV exists on HIP.
        assert_eq!(gemv_kind(Residency::DequantBf16), None);
        assert_eq!(gemv_kind(Residency::DequantF32), None);
        assert_eq!(gemv_kind(Residency::HostOnly), None);
    }

    #[test]
    fn matmul_bearing_roles() {
        use Dsv4TensorKind::*;
        for kind in [
            OutputHead,
            AttnQA,
            ...
            assert!(is_matmul_bearing(kind, 2, false), "{kind:?}");
        }
        // HC mixer matrix is matmul-bearing; its base/scale vectors are not.
        assert!(is_matmul_bearing(HyperConnection, 2, false));
        assert!(!is_matmul_bearing(HyperConnection, 1, false));
        assert!(is_matmul_bearing(HeadHyperConnection, 2, false));
        assert!(!is_matmul_bearing(HeadHyperConnection, 1, false));
    ...
    #[test]
    fn mutated_buffer_table_is_complete() {
        assert_eq!(ATTENTION_LAUNCHER_WRITES.len(), 6);
        for row in ATTENTION_LAUNCHER_WRITES {
            assert!(!row.writes.is_empty(), "{} has no writes", row.launcher);
        }
        // Every persistent slot buffer family appears exactly where the
        // CUDA call sites mutate it: SW ring by both attention kernels,
        // the five compressor + five indexer buffers by compressor_update.
        let slot_writes: Vec<&str> = ATTENTION_LAUNCHER_WRITES
            .iter()
            .flat_map(|r| r.writes.iter().copied())
            .filter(|w| w.starts_with("slot."))
            .collect();
        assert_eq!(
            slot_writes
                .iter()
                .filter(|w| **w == "slot.sw_window_ring")
                .count(),
            2
        );
        for family in ["compressor", "indexer"] {
            for buf in [
                "pending_kv",
                "pending_score",
                "prev_overlap_kv",
                ...
                let name = format!("slot.{family}.{buf}");
                assert!(slot_writes.iter().any(|w| *w == name), "{name} missing");
            }
        }
    }

    #[test]
    fn slot_offsets_are_additive_exact() {
        let cfg = small_config();
        let shape = Dsv4SlotShape::new(&cfg, 1024);
        let offs = slot_layer_offsets(&shape, (cfg.head_dim, cfg.index_head_dim));
        assert_eq!(offs.len(), 3);
        // Layer 0: SW only at offset 0.
        assert_eq!(offs[0].sw_window, 0);
        assert!(offs[0].compressor.is_none() && offs[0].indexer.is_none());
        // Layer 1 starts after layer 0's ring; family fields are contiguous.
        assert_eq!(offs[1].sw_window, 2048);
        let c = offs[1].compressor.unwrap();
        assert_eq!(c.pending_kv, 2048 + 2048);
        assert_eq!(c.pending_score, c.pending_kv + 512);
        assert_eq!(c.prev_overlap_kv, c.pending_score + 512);
        assert_eq!(c.prev_overlap_score, c.prev_overlap_kv + 256);
        assert_eq!(c.compressed, c.prev_overlap_score + 256);
        assert_eq!(c.compressed_rows, 256); // ceil(1024/4)
        let i = offs[1].indexer.unwrap();
        assert_eq!(i.pending_kv, c.compressed + 16384);
        // Layer 2 starts exactly at layer0+layer1 totals; final cursor = total.
        assert_eq!(offs[2].sw_window, 2048 + 2048 + 17920 + 17920);
        let c2 = offs[2].compressor.unwrap();
        assert_eq!(
            c2.compressed + 2048,
            shape.total_elems(),
            "arena layout must cover total_elems exactly"
        );
    }
    ...
    #[test]
    fn routing_matches_sqrtsoftplus_formula() {
        // 16 experts, top-2, scale 1.5, layer 1 (bias-routed), per
        // deepseek4.cpp build_moe_v4 (scores=sqrt(softplus), select on
        // scores+bias, weights=scores normalized then scaled).
        let cfg = small_config();
        let mut logits = vec![0.0f32; 16];
        logits[3] = 2.0;
        logits[7] = 1.5;
        logits[9] = 1.4;
        // Bias pushes expert 9 past BOTH 7 and 3 in SELECTION only; weights
        // come from the unbiased scores.
        let mut bias = vec![0.0f32; 16];
        bias[9] = 0.5;
        let routes = route_layer_token(&cfg, 1, 0, &logits, Some(&bias), None).unwrap();
        assert_eq!(routes.len(), 2);
        assert_eq!(routes[0].0, 9);
        assert_eq!(routes[1].0, 3);
        let s3 = softplus(2.0).sqrt();
        let s9 = softplus(1.4).sqrt();
        let denom = s3 + s9 + 1.0e-9;
        assert!((routes[0].1 - s9 / denom * 1.5).abs() < 1e-6);
        assert!((routes[1].1 - s3 / denom * 1.5).abs() < 1e-6);
        // Weights normalize before scaling: sum = scaling factor.
        let sum: f32 = routes.iter().map(|r| r.1).sum();
        assert!((sum - 1.5).abs() < 1e-5);
    }

    #[test]
    fn routing_hash_layer_uses_tid2eid() {
        let cfg = small_config(); // num_hash_layers = 1, topk = 2
        let logits = vec![1.0f32; 16];
        let mut tid2eid = vec![0i64; 2 * 1000];
        tid2eid[42 * 2] = 5;
        tid2eid[42 * 2 + 1] = 11;
        let routes = route_layer_token(&cfg, 0, 42, &logits, None, Some(&tid2eid)).unwrap();
        assert_eq!(routes.iter().map(|r| r.0).collect::<Vec<_>>(), vec![5, 11]);
        // Uniform scores → equal weights summing to the scaling factor.
        assert!((routes[0].1 - routes[1].1).abs() < 1e-6);
        assert!((routes[0].1 + routes[1].1 - 1.5).abs() < 1e-5);
        // Hash layer without a table is a clear error.
        assert!(route_layer_token(&cfg, 0, 42, &logits, None, None).is_err());
        // Bias layer without a bias is a clear error.
        assert!(route_layer_token(&cfg, 1, 42, &logits, None, None).is_err());
    }
}
