...
mod tests {
    ...
    #[test]
    fn slot_shape_hand_computed() {
        let shape = Dsv4SlotShape::new(&small_config(), 1024);
        assert_eq!(shape.layers.len(), 3);

        // Layer 0 (SW): only the window ring = 32*64.
        assert_eq!(shape.layers[0].sw_window, 2048);
        assert_eq!(shape.layers[0].compressor, None);
        assert_eq!(shape.layers[0].indexer, None);
        assert_eq!(shape.layers[0].total_elems(), 2048);

        // Layer 1 (CSA, ratio 4, overlap): width=128 → pending 512 ×2,
        // prev 256 ×2, compressed 64*ceil(1024/4)=16384 → 17920; indexer
        // identical (index_head_dim=64) → 17920.
        let comp = shape.layers[1].compressor.unwrap();
        assert_eq!(comp.pending_kv, 512);
        assert_eq!(comp.prev_overlap_kv, 256);
        assert_eq!(comp.compressed, 16384);
        assert_eq!(comp.total(), 17920);
        assert_eq!(shape.layers[1].indexer.unwrap().total(), 17920);
        assert_eq!(shape.layers[1].total_elems(), 2048 + 17920 + 17920);

        // Layer 2 (HCA, ratio 32, no overlap): width=64 → pending 2048 ×2,
        // prev 2048 ×2, compressed 64*32=2048 → 10240; no indexer.
        let comp = shape.layers[2].compressor.unwrap();
        assert_eq!(comp.pending_kv, 2048);
        assert_eq!(comp.prev_overlap_kv, 2048);
        assert_eq!(comp.compressed, 2048);
        assert_eq!(comp.total(), 10240);
        assert_eq!(shape.layers[2].indexer, None);
        assert_eq!(shape.layers[2].total_elems(), 2048 + 10240);

        assert_eq!(shape.total_elems(), 2048 + 37888 + 12288);
        assert_eq!(shape.total_bytes(), 52224 * 2);
    }

    #[test]
    fn pool_alloc_free_and_prefix_retain() {
        let cfg = small_config();
        let mut pool = HipKvPool::new(&cfg, 2, 8, 16, 1024);
        let dyn_pool: &mut dyn KvPool = &mut pool;
        assert!(dyn_pool.is_active());
        assert_eq!(dyn_pool.page_size(), 16);
        dyn_pool.alloc(0, 32).unwrap();
        assert_eq!(dyn_pool.seq_len(0), 32);
        assert_eq!(dyn_pool.page_indices(0).len(), 2);

        let prefix: Vec<u32> = dyn_pool.page_indices(0).to_vec();
        dyn_pool.retain_pages(&prefix);
        let before = dyn_pool.free_pages();
        dyn_pool.free_slot(0);
        assert_eq!(dyn_pool.free_pages(), before);
        dyn_pool.attach_pages(1, &prefix, 32).unwrap();
        assert_eq!(dyn_pool.seq_len(1), 32);
        dyn_pool.free_slot(1);
        dyn_pool.release_pages(&prefix);
        assert_eq!(dyn_pool.retained_count(), 0);
        assert_eq!(dyn_pool.free_pages(), 8);
    }

    #[test]
    fn pool_rejects_overflow() {
        let cfg = small_config();
        let mut pool = HipKvPool::new(&cfg, 1, 4, 16, 48);
        assert!(pool.alloc(0, 49).is_err());
