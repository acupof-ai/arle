...
mod tests {
    ...
    /// Full per-layer name table extracted from llama-arch.cpp:345-571 +
    /// the deepseek4 create_tensor calls (llama-model.cpp:5441-5567).
    const LAYER_NAMES: &[(&str, Dsv4TensorKind)] = {
        use Dsv4TensorKind::*;
        &[
            ("attn_norm.weight", AttnNorm),
            ("ffn_norm.weight", FfnNorm),
            ...
            ("indexer.compress_kv.weight", IndexerCompressKv),
            ("indexer.compress_gate.weight", IndexerCompressGate),
            ("hc_attn_base", HyperConnection),
            ("hc_attn_fn", HyperConnection),
            ("hc_attn_scale", HyperConnection),
            ("hc_ffn_base", HyperConnection),
            ("hc_ffn_fn", HyperConnection),
            ("hc_ffn_scale", HyperConnection),
            ("ffn_gate_inp.weight", RouterGate),
            ("exp_probs_b", RouterBias),
            ("ffn_gate_tid2eid", RouterHashTable),
    ...
    #[test]
    fn every_extracted_name_maps_to_a_role() {
        for (global, kind) in [
            ("token_embd.weight", Dsv4TensorKind::TokenEmbedding),
            ("output_norm.weight", Dsv4TensorKind::OutputNorm),
            ("output.weight", Dsv4TensorKind::OutputHead),
            ("hc_head_base", Dsv4TensorKind::HeadHyperConnection),
            ("hc_head_fn", Dsv4TensorKind::HeadHyperConnection),
            ("hc_head_scale", Dsv4TensorKind::HeadHyperConnection),
        ] {
            let role = classify_tensor(global).unwrap();
            assert_eq!(role, Dsv4TensorRole { layer: None, kind }, "{global}");
        }
        for layer in [0usize, 7, 60] {
            for (suffix, kind) in LAYER_NAMES {
                let name = format!("blk.{layer}.{suffix}");
                let role = classify_tensor(&name).unwrap();
                assert_eq!(
                    role,
                    Dsv4TensorRole {
                        layer: Some(layer),
    ...
    fn unknown_names_rejected() {
        for bad in [
            "lm_head.weight",
            "blk.0.attn_q.weight",
            "blk.x.attn_norm.weight",
            "blk.0.ffn_gate.weight",
            "blk.0",
            "",
        ] {
            assert!(classify_tensor(bad).is_err(), "{bad:?} should be unknown");
        }
    }

    fn synthetic_dsv4_gguf() -> std::path::PathBuf {
        let kvs = vec![
            ("general.architecture", V::Str("deepseek4")),
            ("deepseek4.block_count", V::U32(4)),
            ("deepseek4.context_length", V::U32(4096)),
            ("deepseek4.embedding_length", V::U32(512)),
            ("deepseek4.vocab_size", V::U32(1000)),
            ("deepseek4.attention.head_count", V::U32(8)),
            ...
            ("deepseek4.attention.indexer.head_count", V::U32(4)),
            ("deepseek4.attention.indexer.key_length", V::U32(64)),
            ("deepseek4.attention.indexer.top_k", V::U32(128)),
            ("deepseek4.rope.dimension_count", V::U32(32)),
            ("deepseek4.rope.freq_base", V::F32(10000.0)),
            ("deepseek4.rope.freq_base_swa", V::F32(160000.0)),
            ("deepseek4.rope.scaling.type", V::Str("yarn")),
            ("deepseek4.rope.scaling.factor", V::F32(16.0)),
            (
                "deepseek4.rope.scaling.original_context_length",
                V::U32(2048),
            ),
            ("deepseek4.rope.scaling.yarn_beta_fast", V::F32(32.0)),
            ("deepseek4.rope.scaling.yarn_beta_slow", V::F32(1.0)),
            ("deepseek4.expert_count", V::U32(16)),
            ("deepseek4.expert_used_count", V::U32(2)),
            ("deepseek4.expert_shared_count", V::U32(1)),
            ("deepseek4.expert_feed_forward_length", V::U32(256)),
            ("deepseek4.expert_weights_scale", V::F32(1.5)),
            ("deepseek4.expert_weights_norm", V::Bool(true)),
            ("deepseek4.swiglu_clamp_exp", V::ArrF32(vec![10.0; 4])),
            ("tokenizer.ggml.bos_token_id", V::U32(0)),
            ("tokenizer.ggml.eos_token_id", V::U32(1)),
        ];
        let f32t = |name: &str, dims: Vec<u64>| {
            let n: u64 = dims.iter().product();
            T {
                name: name.into(),
                dims,
                type_id: 0,
                data: vec![0u8; n as usize * 4],
            }
        };
        let tensors = vec![
            f32t("token_embd.weight", vec![512, 1000]),
            f32t("hc_head_base", vec![4]),
            f32t("blk.0.attn_output_a.weight", vec![64, 8 * 32]),
            // ratios [0, 4, 0, 32]: ape ne = {width, ratio}.
            f32t("blk.1.attn_compress_ape", vec![128, 4]),
            f32t("blk.3.attn_compress_ape", vec![64, 32]),
            // layer 0 hash-routed.
            f32t("blk.0.ffn_gate_tid2eid", vec![2, 1000]),
        ];
        write_to_temp(&kvs, &tensors, "config")
    }

    #[test]
    fn config_from_synthetic_gguf() {
        let path = synthetic_dsv4_gguf();
        let g = GgufFile::open(&path).unwrap();
        let cfg = dsv4_config_from_gguf(&g).unwrap();
        ...
        assert_eq!(cfg.o_lora_rank, 32);
        assert_eq!(cfg.hc_mult, 4);
        assert_eq!(cfg.compress_ratios, vec![0, 4, 0, 32]);
        assert_eq!(cfg.num_hash_layers, 1);
        assert_eq!(cfg.sliding_window, 32);
        ...
        assert_eq!(cfg.bos_token_id, Some(0));
