...
// Spec-decode TREE build indices variant.
//
// A draft-tree verify chunk packs `n_tokens` tree rows (BFS order) into the
// kv_unified chunk region, but a row may only attend its OWN root→node branch
// inside the chunk — not every earlier row — and rows at the same draft depth
// share an absolute position. So unlike the CSA/HCA kernels:
//   - abs_pos comes from a per-row `positions` array, NOT start_pos + token;
//   - the in-chunk window part is `ancestors[token]` (chunk-row indices,
//     root included, -1 padded) + the row itself, NOT [0, token).
// The committed SW-cache window part and the compressed part (CSA selected
// translate / HCA causal identity / none for pure-SW layers) mirror the
// existing kernels per-line.
//
// Row layout:
//   [0, committed)                          ← SW cache offsets for
//                                             [max(0, pos+1-sw_window), start_pos)
//   [committed, committed + anc + 1)        ← ancestor chunk rows + self
//   [.., .. + comp_part)                    ← CSA: selected translate;
//                                             HCA: identity causal; SW: none
//   tail                                    ← -1 padding
// topk_length = committed + anc + 1 + comp_part.
__global__ void arle_tree_build_indices_kernel(
        int32_t* __restrict__ indices,        // [s_q, topk_unified]
        int32_t* __restrict__ topk_length,    // [s_q]
        const int32_t* __restrict__ positions,// [s_q] absolute positions
        const int32_t* __restrict__ ancestors,// [s_q, max_anc] chunk rows, -1 pad
        int max_anc,
        const int32_t* __restrict__ selected, // [s_q, index_topk] or nullptr
        int s_q,
        int start_pos,
        int sw_window,
        int index_topk,        // selected width (CSA); 0 otherwise
        int max_compressed,    // HCA identity cap; 0 otherwise
        int topk_unified,
        int n_tokens,          // = s_q (chunk rows)
        int compressed_count,
        int compress_ratio) {
    int token = blockIdx.x;
    if (token >= s_q) return;
    ...
    // [0, committed): committed SW-cache offsets (rolling region rebased,
    // same arithmetic as the CSA/HCA kernels' p < start_pos arm).
    for (int j = threadIdx.x; j < committed; j += blockDim.x) {
        int p = sw_start + j;
        row[j] = p - sw_base;
    }

    // [committed, committed + anc + 1): this row's branch inside the chunk —
    // ancestors (root included) then self. Serial per row (anc <= max_anc <= 8).
    int anc = 0;
    if (threadIdx.x == 0) {
        ...
        row[committed + anc] = sw_window + token;  // self
    }
    __syncthreads();
    // Recompute anc on every thread for the offsets below (cheap, register-only).
    {
        int count = 0;
        const int32_t* anc_row = ancestors + (size_t)token * max_anc;
        for (int j = 0; j < max_anc; ++j) {
    ...
    int comp_part = 0;
    if (selected != nullptr) {
        // CSA: selected translate + causality gate (mirrors
        // arle_csa_build_indices_kernel per-line).
        ...
        // HCA: identity causal (mirrors arle_hca_build_indices_kernel).
        int comp_keys = (compress_ratio > 0) ? (abs_pos / compress_ratio) : 0;
        if (comp_keys > compressed_count) comp_keys = compressed_count;
        if (comp_keys < 0) comp_keys = 0;
        comp_part = comp_keys;
...
// Spec-decode TREE indices launcher. `positions`/`ancestors` are device
// arrays (per-row absolute position; per-row branch chunk-rows, -1 padded,
// root included, self implicit). Exactly one of (`selected` non-null,
// index_topk > 0) / (max_compressed > 0) / neither selects CSA / HCA /
// pure-SW compressed handling. `topk_unified` is caller-sized:
// sw_window + tree_pad(128) + (index_topk | max_compressed | 0), %128 == 0.
cudaError_t arle_flashmla_tree_build_indices(
        int32_t* indices,
        int32_t* topk_length,
    ...
    if ((topk_unified & 127) != 0) return cudaErrorInvalidValue;
    // Capacity: committed(<=sw_window) + branch(<=max_anc+1) + compressed part.
    const int comp_cap = (selected != nullptr) ? index_topk : max_compressed;
    if (sw_window + max_anc + 1 + comp_cap > topk_unified) {
        return cudaErrorInvalidValue;
    }
    constexpr int kBlock = 128;
    arle_tree_build_indices_kernel<<<s_q, kBlock, 0, stream>>>(
