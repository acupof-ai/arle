// Vendored from sgl-project/sglang sgl-kernel/csrc/allreduce/custom_all_reduce.cuh
// (Apache-2.0), itself adapted from vllm-project/vllm v0.8.2 csrc/custom_all_reduce.cuh.
// ARLE adaptations: USE_MUSA branches resolved away (CUDA-only), `#include "utils.h"`
// replaced with an inline CHECK_CUDA_SUCCESS, everything else kept byte-close to
// upstream so future re-syncs diff cleanly. The C ABI wrapper (ARLE-authored) lives
// in custom_all_reduce.cu next to this header.
//
// One-shot (1stage) properties this bench relies on: fixed cross-rank accumulation
// order => bitwise-identical results on every rank (lockstep requirement), and
// graph-capture support via deferred RankData slots.
