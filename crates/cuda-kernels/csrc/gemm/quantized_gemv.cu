...
// f32-block-scale variant of dsv4_fp8_grouped_gemv_batch_kernel: the DSv4
// MoE expert caches store DeepGEMM block scales as arbitrary f32 (NOT UE8M0
// — that encoding is checkpoint-side for the attention tensors only), so the
// decode-band GEMV lane reads them directly.
__global__ void dsv4_fp8_grouped_gemv_batch_f32s_kernel(
    const uint64_t* __restrict__ weight_ptrs,
    const uint64_t* __restrict__ scale_ptrs,
    const __nv_bfloat16* __restrict__ input,
    __nv_bfloat16* __restrict__ output,
...
// f32-block-scale variant of dsv4_fp8_grouped_gemv_pair_batch_kernel.
__global__ void dsv4_fp8_grouped_gemv_pair_batch_f32s_kernel(
    const uint64_t* __restrict__ weight_a_ptrs,
    const uint64_t* __restrict__ scale_a_ptrs,
    const uint64_t* __restrict__ weight_b_ptrs,
    const uint64_t* __restrict__ scale_b_ptrs,
    ...
    int scale_cols)
{
    ...
    for (int k = tid_in_row; k < K; k += threads_per_row) {
        ...
        const float xv = __bfloat162float(x[k]);
        const float wa = dsv4_decode_fp8_e4m3(weight_a[row * K + k])
            * scales_a[scale_row_offset + sc];
        const float wb = dsv4_decode_fp8_e4m3(weight_b[row * K + k])
            * scales_b[scale_row_offset + sc];
        sum_a += wa * xv;
        sum_b += wb * xv;
    }
...
cudaError_t dsv4_fp8_grouped_gemv_batch_f32s_cuda(
    const uint64_t* weight_ptrs,
    const uint64_t* scale_ptrs,
    const __nv_bfloat16* input,
    __nv_bfloat16* output,
    ...
    dim3 block(GEMV_THREADS);
    dim3 grid((N + GEMV_ROWS - 1) / GEMV_ROWS, max_count, num_experts);
    dsv4_fp8_grouped_gemv_batch_f32s_kernel<<<grid, block, 0, stream>>>(
        weight_ptrs, scale_ptrs, input, output, offsets, counts, expert_indices,
        max_count, N, K, scale_rows, scale_cols);
    return cudaGetLastError();
}

cudaError_t dsv4_fp8_grouped_gemv_pair_batch_f32s_cuda(
    ...
    dim3 block(GEMV_THREADS);
    dim3 grid((N + GEMV_ROWS - 1) / GEMV_ROWS, max_count, num_experts);
    dsv4_fp8_grouped_gemv_pair_batch_f32s_kernel<<<grid, block, 0, stream>>>(
        weight_a_ptrs, scale_a_ptrs, weight_b_ptrs, scale_b_ptrs, input,
        output_a, output_b, offsets, counts, expert_indices,
        max_count, N, K, scale_rows, scale_cols);
    return cudaGetLastError();
}
