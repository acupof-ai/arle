...
- **Micro-kernel knobs (launch-shape sweep, vector-load, scale/row-pointer hoist, persistent
  kernel) are sub-noise on tiny operators.** ~25 P3 DSv4 micro-opt kills share the same
  lesson: a 0.5–2% kernel-level delta on a < 50 µs kernel doesn't survive the
