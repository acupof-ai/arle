...
extern "C" __global__ void __launch_bounds__(128, 1) kernel_kernel(const int* __restrict__ KV_indices, const int* __restrict__ KV_indptr, const int* __restrict__ KV_last_page_len, const bfloat16_t* __restrict__ K_pool, bfloat16_t* __restrict__ Output, const bfloat16_t* __restrict__ Q, const int* __restrict__ Q_indptr, const bfloat16_t* __restrict__ V_pool, int batch_size, int batch_size_1, int batch_size_plus_one, int batch_size_plus_one_1, int max_qlen, int num_pages, int num_pages_1, int total_pages, int total_q_tokens, int total_q_tokens_1) {
  ...
  void* v_tile = ((void*)((char*)buf_dyn_shmem + 16384));
  float acc_o[32];
  float m_i[2];
  float l_i[2];
  int in_page_j[1];
  ...
  *(float2*)(l_i + 0) = make_float2(broadcast_var_2, broadcast_var_2);
  ...
  for (int i_1 = 0; i_1 < 4; ++i_1) {
    ...
    uint4 condval_5;
    if (((((((((int)blockIdx.x) * 64) + (i_1 * 16)) + (((int)threadIdx.x) >> 3)) < (q_end - q_start)) && (0 <= ((((((int)blockIdx.x) * 64) + (i_1 * 16)) + (((int)threadIdx.x) >> 3)) + q_start))) && (((((((int)blockIdx.x) * 64) + (i_1 * 16)) + (((int)threadIdx.x) >> 3)) + q_start) < total_q_tokens))) {
      condval_5 = *(uint4*)(Q + ((((((((int64_t)((int)blockIdx.x)) * (int64_t)65536) + (((int64_t)i_1) * (int64_t)16384)) + ((((int64_t)((int)threadIdx.x)) >> (int64_t)3) * (int64_t)1024)) + (((int64_t)q_start) * (int64_t)1024)) + (((int64_t)((int)blockIdx.y)) * (int64_t)64)) + ((((int64_t)((int)threadIdx.x)) & (int64_t)7) * (int64_t)8)));
    } else {
    ...
    *(uint4*)(((bfloat16_t*)q_tile) + (((((i_1 * 1024) + ((((int)threadIdx.x) >> 3) * 64)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 31) >> 4) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 15) >> 3) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_5;
  }
  int condval_7;
  ...
  for (int kn = 0; kn < ((condval_6 + 63) >> 6); ++kn) {
    in_page_j[0] = (((int)threadIdx.x) & 15);
    int condval_9;
    if (((((kn * 64) + (((int)threadIdx.x) & 63)) + 16) < (((kv_page_end * 16) + last_page_len) - (kv_page_start * 16)))) {
      condval_9 = 1;
    } else {
      condval_9 = 0;
    }
    valid_j[0] = condval_9;
    int condval_10;
    if (((((((kn * 64) + (((int)threadIdx.x) & 63)) + 16) < (((kv_page_end * 16) + last_page_len) - (kv_page_start * 16))) && (0 <= (((kn * 4) + ((((int)threadIdx.x) & 63) >> 4)) + kv_page_start))) && ((((kn * 4) + ((((int)threadIdx.x) & 63) >> 4)) + kv_page_start) < total_pages))) {
      condval_10 = KV_indices[(((((int64_t)kn) * (int64_t)4) + ((((int64_t)((int)threadIdx.x)) & (int64_t)63) >> (int64_t)4)) + ((int64_t)kv_page_start))];
    } else {
      condval_10 = 0;
    }
    page_idx_j[0] = condval_10;
    if ((((int)threadIdx.x) >> 6) == 0) {
      __syncthreads();
      #pragma unroll
      for (int i_2 = 0; i_2 < 8; ++i_2) {
        bool is_valid = (valid_j[0] != 0);
        bfloat16_t broadcast_var_4 = bfloat16_t(0x0p+0f/*0.000000e+00*/);
        uint4 condval_11;
        if (((((is_valid && (0 <= in_page_j[0])) && (in_page_j[0] < 16)) && (0 <= page_idx_j[0])) && (page_idx_j[0] < num_pages))) {
          condval_11 = *(uint4*)(K_pool + (((((int64_t)page_idx_j[(int64_t)0]) * (int64_t)1024) + (((int64_t)in_page_j[(int64_t)0]) * (int64_t)64)) + (((int64_t)i_2) * (int64_t)8)));
        } else {
          condval_11 = make_uint4(__pack_nv_bfloat162(broadcast_var_4, broadcast_var_4), __pack_nv_bfloat162(broadcast_var_4, broadcast_var_4), __pack_nv_bfloat162(broadcast_var_4, broadcast_var_4), __pack_nv_bfloat162(broadcast_var_4, broadcast_var_4));
        }
        *(uint4*)(((bfloat16_t*)k_tile) + (((((((int)threadIdx.x) & 63) * 64) + ((((i_2 >> 2) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((i_2 & 3) >> 1) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + ((((i_2 & 1) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_11;
        bfloat16_t broadcast_var_5 = bfloat16_t(0x0p+0f/*0.000000e+00*/);
        uint4 condval_12;
        if (((((is_valid && (0 <= in_page_j[0])) && (in_page_j[0] < 16)) && (0 <= page_idx_j[0])) && (page_idx_j[0] < num_pages_1))) {
          condval_12 = *(uint4*)(V_pool + (((((int64_t)page_idx_j[(int64_t)0]) * (int64_t)1024) + (((int64_t)in_page_j[(int64_t)0]) * (int64_t)64)) + (((int64_t)i_2) * (int64_t)8)));
        } else {
          condval_12 = make_uint4(__pack_nv_bfloat162(broadcast_var_5, broadcast_var_5), __pack_nv_bfloat162(broadcast_var_5, broadcast_var_5), __pack_nv_bfloat162(broadcast_var_5, broadcast_var_5), __pack_nv_bfloat162(broadcast_var_5, broadcast_var_5));
        }
        *(uint4*)(((bfloat16_t*)v_tile) + (((((((int)threadIdx.x) & 63) * 64) + ((((i_2 >> 2) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((i_2 & 3) >> 1) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + ((((i_2 & 1) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_12;
      }
    }
    #pragma unroll
    ...
    for (int i_5 = 0; i_5 < 32; ++i_5) {
      ...
      if ((((((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_5 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)) < (q_end - q_start)) && ((((((kn * 64) + ((i_5 >> 2) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_5 & 1)) + 16) < (((kv_page_end * 16) + last_page_len) - (kv_page_start * 16)))) && ((((((kn * 64) + ((i_5 >> 2) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_5 & 1)) + 16) <= (((((((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + (kv_page_end * 16)) + (((i_5 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)) + last_page_len) + q_start) - q_end) - (kv_page_start * 16))))) {
        condval_13 = (scores[i_5] * 0x1p-3f/*1.250000e-01*/);
      } else {
        condval_13 = -CUDART_INF_F;
      }
      scores[i_5] = condval_13;
    }
      ...
      *(uint1*)(Output_local_cast + 0) = __2;
      if (0 <= (((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + ((i_15 & 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)) + q_start)) {
        if ((((((((int)blockIdx.x) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + ((i_15 & 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)) + q_start) < total_q_tokens_1) {
          *(uint1*)(Output + ((((((((((int64_t)((int)blockIdx.x)) * (int64_t)65536) + ((((int64_t)((int)threadIdx.x)) >> (int64_t)5) * (int64_t)16384)) + ((((int64_t)i_15) & (int64_t)1) * (int64_t)8192)) + (((((int64_t)((int)threadIdx.x)) & (int64_t)31) >> (int64_t)2) * (int64_t)1024)) + (((int64_t)q_start) * (int64_t)1024)) + (((int64_t)((int)blockIdx.y)) * (int64_t)64)) + ((((int64_t)i_15) >> (int64_t)1) * (int64_t)8)) + ((((int64_t)((int)threadIdx.x)) & (int64_t)3) * (int64_t)2))) = *(uint1*)(Output_local_cast + 0);
        }
      }
    }
  }
