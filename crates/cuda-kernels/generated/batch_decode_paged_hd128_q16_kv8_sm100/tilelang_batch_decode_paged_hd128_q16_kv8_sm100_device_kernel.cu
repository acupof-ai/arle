...
extern "C" __global__ void __launch_bounds__(128, 1) kernel_kernel(const int* __restrict__ KV_indices, const int* __restrict__ KV_indptr, const int* __restrict__ KV_last_page_len, const bfloat16_t* __restrict__ K_pool, bfloat16_t* __restrict__ Output, const bfloat16_t* __restrict__ Q, const bfloat16_t* __restrict__ V_pool, int batch_size, int batch_size_1, int batch_size_plus_one, int num_pages, int num_pages_1, int total_pages, int total_q_tokens, int total_q_tokens_1) {
  ...
  *(float2*)(l_i + 0) = make_float2(broadcast_var_2, broadcast_var_2);
  ...
  for (int i_1 = 0; i_1 < 8; ++i_1) {
    ...
    uint4 condval_3;
    if (((((i_1 * 8) + (((int)threadIdx.x) >> 4)) == 0) && (((int)blockIdx.z) < total_q_tokens))) {
      condval_3 = *(uint4*)(Q + (((((int64_t)((int)blockIdx.z)) * (int64_t)2048) + (((int64_t)((int)blockIdx.y)) * (int64_t)128)) + ((((int64_t)((int)threadIdx.x)) & (int64_t)15) * (int64_t)8)));
    } else {
  ...
  for (int kn = 0; kn < ((((last_page_len - 1) >> 4) + kv_page_end) - kv_page_start); ++kn) {
    ...
    for (int i_2 = 0; i_2 < 2; ++i_2) {
      int condval_4;
      if ((((((((kn * 16) + (i_2 * 8)) + (((int)threadIdx.x) >> 4)) + 16) < (((kv_page_end * 16) + last_page_len) - (kv_page_start * 16))) && (0 <= (kv_page_start + kn))) && ((kv_page_start + kn) < total_pages))) {
      ...
      uint4 condval_5;
      if ((((((((kn * 16) + (i_2 * 8)) + (((int)threadIdx.x) >> 4)) + 16) < (((kv_page_end * 16) + last_page_len) - (kv_page_start * 16))) && (0 <= page_idx)) && (page_idx < num_pages))) {
        condval_5 = *(uint4*)(K_pool + ((((((int64_t)page_idx) * (int64_t)16384) + ((((int64_t)((int)blockIdx.y)) >> (int64_t)1) * (int64_t)2048)) + (((int64_t)i_2) * (int64_t)1024)) + (((int64_t)((int)threadIdx.x)) * (int64_t)8)));
      } else {
      ...
      *(uint4*)(((bfloat16_t*)k_tile) + ((((((((((int)threadIdx.x) & 15) >> 3) * 1024) + (i_2 * 512)) + ((((int)threadIdx.x) >> 4) * 64)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_5;
      ...
      uint4 condval_6;
      if ((((((((kn * 16) + (i_2 * 8)) + (((int)threadIdx.x) >> 4)) + 16) < (((kv_page_end * 16) + last_page_len) - (kv_page_start * 16))) && (0 <= page_idx)) && (page_idx < num_pages_1))) {
        condval_6 = *(uint4*)(V_pool + ((((((int64_t)page_idx) * (int64_t)16384) + ((((int64_t)((int)blockIdx.y)) >> (int64_t)1) * (int64_t)2048)) + (((int64_t)i_2) * (int64_t)1024)) + (((int64_t)((int)threadIdx.x)) * (int64_t)8)));
      } else {
    ...
    if (((((((int)threadIdx.x) >> 5) * 16) + ((i_14 & 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)) == 0) {
      ...
      *(uint1*)(Output_local_cast + 0) = __2;
      if (((int)blockIdx.z) < total_q_tokens_1) {
        *(uint1*)(Output + ((((((int64_t)((int)blockIdx.z)) * (int64_t)2048) + (((int64_t)((int)blockIdx.y)) * (int64_t)128)) + ((((int64_t)i_14) >> (int64_t)1) * (int64_t)8)) + ((((int64_t)((int)threadIdx.x)) & (int64_t)3) * (int64_t)2))) = *(uint1*)(Output_local_cast + 0);
      }
    }
  }
}
