...
CUresult tilelang_batch_prefill_paged_hd64_q16_kv1_run_cuda(uint16_t *q, const int32_t *q_indptr, uint16_t *k_pool, uint16_t *v_pool, const int32_t *kv_indptr, const int32_t *kv_indices, const int32_t *kv_last_page_len, uint16_t *o, int32_t batch_size, int32_t total_q_tokens, int32_t max_qlen, int32_t num_pages, int32_t total_pages, int32_t num_q_heads, int32_t num_kv_heads, int32_t page_size, float sm_scale, CUstream stream) {
    int sm = g_sm_pack;
    if (sm < 0) {
        sm = load_sm_pack();
    ...
    switch (sm) {
        case 100: return tilelang_batch_prefill_paged_hd64_q16_kv1_run_sm100_cuda(q, q_indptr, k_pool, v_pool, kv_indptr, kv_indices, kv_last_page_len, o, batch_size, total_q_tokens, max_qlen, num_pages, total_pages, num_q_heads, num_kv_heads, page_size, sm_scale, stream);
        case 120: return tilelang_batch_prefill_paged_hd64_q16_kv1_run_sm120_cuda(q, q_indptr, k_pool, v_pool, kv_indptr, kv_indices, kv_last_page_len, o, batch_size, total_q_tokens, max_qlen, num_pages, total_pages, num_q_heads, num_kv_heads, page_size, sm_scale, stream);
