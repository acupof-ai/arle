...
#include "sm90_mega_moe_core.hpp"
...
static bool should_use_swap_ab_for_mega_moe_sm90(
    ...
    return should_use_swap_ab_for_mega_moe_sm90(
        num_experts_per_rank, num_tokens, num_topk, block_m,
        num_epilogue_threads, get_env<int>("DG_SM90_FP8_SWAP_AB", 1) != 0);
}
...
static MegaMoESM90Config get_mega_moe_config_sm90(
    ...
    const auto config = get_mega_moe_config_sm90_core(
        ...
        device_runtime->get_num_sms(), SM90ArchSpec::smem_capacity,
    ...
    if (get_env<int>("DG_JIT_DEBUG") or get_env<int>("DG_PRINT_CONFIGS")) {
        ...
        const auto key = fmt::format(
            ...
            num_max_tokens_per_rank, num_tokens, num_topk, use_swap_ab);
        static std::unordered_set<std::string> printed;
