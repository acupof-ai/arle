...
fn main() -> anyhow::Result<()> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "usage: cargo run -p cli --example metal_kv_memory_probe --release --no-default-features --features metal,no-cuda -- [--model MODEL] [--kv-cache-dtype auto|bf16|int8] [--prompt-tokens N] [--max-tokens N] [--memory-budget-gib N] [--low-impact true|false] [--warmup-runs N] [--repeat N]"
                );
                return Ok(());
            }
