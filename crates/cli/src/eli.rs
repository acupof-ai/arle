...
mod tests {
    ...
    #[test]
    fn config_roundtrip_full() {
        let cfg = AgentConfig {
            agent: Some("eli".into()),
            model: Some("mlx-community/Qwen3.5-0.8B-MLX-4bit".into()),
            mode: Some("chat".into()),
        };
        let text = cfg.to_toml();
        let parsed = AgentConfig::from_toml(&text);
        assert_eq!(cfg, parsed);
        assert!(parsed.selects_eli());
        assert_eq!(parsed.launch_mode(), EliMode::Chat);
    }

    #[test]
    fn config_roundtrip_gateway_mode() {
        let cfg = AgentConfig {
            agent: Some("eli".into()),
            model: Some("/abs/path/to/model".into()),
            mode: Some("gateway".into()),
        };
        let parsed = AgentConfig::from_toml(&cfg.to_toml());
        assert_eq!(parsed.launch_mode(), EliMode::Gateway);
        assert!(parsed.selects_eli());
    }

    #[test]
    fn config_partial_and_comments_parse() {
        let text = "# arle agent config\nagent = \"eli\"\n\n# no model yet\n";
        let cfg = AgentConfig::from_toml(text);
        assert_eq!(cfg.agent.as_deref(), Some("eli"));
        assert_eq!(cfg.model, None);
        // agent=eli but no model → not launch-ready.
        assert!(!cfg.selects_eli());
    }
    ...
    fn config_empty_is_default() {
        ...
        assert_eq!(cfg.launch_mode(), EliMode::Chat);
    }

    #[test]
    fn config_non_eli_agent_not_selected() {
        let cfg = AgentConfig {
            agent: Some("arle".into()),
    ...
    #[test]
    fn config_blank_model_not_selected() {
        let cfg = AgentConfig {
            agent: Some("eli".into()),
            model: Some("   ".into()),
            mode: Some("chat".into()),
    ...
    fn config_escapes_quotes_and_backslashes() {
        let cfg = AgentConfig {
            agent: Some("eli".into()),
            model: Some(r#"weird"path\with-meta"#.into()),
            mode: Some("chat".into()),
        };
        let parsed = AgentConfig::from_toml(&cfg.to_toml());
        assert_eq!(parsed.model.as_deref(), Some(r#"weird"path\with-meta"#));
    }

    #[test]
    fn find_free_port_returns_bindable_port() {
        // The returned port must lie in the scan window.
        let port = find_free_port(PORT_SCAN_BASE, PORT_SCAN_SPAN).expect("a free port exists");
        assert!((PORT_SCAN_BASE..PORT_SCAN_BASE + PORT_SCAN_SPAN).contains(&port));
    }

    #[test]
    fn find_free_port_skips_occupied_port() {
        // Bind an OS-assigned ephemeral port FIRST (port 0), then anchor the
        // scan at that exact port. The scan must skip the held port. Using a
        // pre-held OS-assigned port avoids a TOCTOU race with the bindable-port
        // test running in parallel (which could otherwise claim the same base).
    ...
    #[test]
    fn find_free_port_empty_span_errors() {
        assert!(find_free_port(PORT_SCAN_BASE, 0).is_err());
    }

    #[test]
    fn eli_env_has_local_routing() {
        let env = eli_env(8123, "Qwen3.5-0.8B-MLX-4bit");
        let map: std::collections::HashMap<_, _> = env.into_iter().collect();
        assert_eq!(map["ELI_API_BASE"], "http://127.0.0.1:8123/v1");
        assert_eq!(map["ELI_API_KEY"], "local");
        assert_eq!(map["ELI_MODEL"], "local:Qwen3.5-0.8B-MLX-4bit");
        assert_eq!(map["ELI_API_FORMAT"], "completion");
        // Fallback disabled (empty) → no cross-provider drift.
        assert_eq!(map["ELI_FALLBACK_MODELS"], "");
    }

    #[test]
    fn parse_first_model_id_extracts_served_id() {
        let resp = "HTTP/1.1 200 OK\r\nContent-Type: application/json\r\n\r\n\
            {\"object\":\"list\",\"data\":[{\"id\":\"Qwen3.5-0.8B-MLX-4bit\",\"object\":\"model\"}]}";
        assert_eq!(
            parse_first_model_id(resp).as_deref(),
            Some("Qwen3.5-0.8B-MLX-4bit")
        );
    }

    #[test]
    fn parse_first_model_id_none_when_absent() {
        let resp = "HTTP/1.1 200 OK\r\n\r\n{\"object\":\"list\",\"data\":[]}";
        assert_eq!(parse_first_model_id(resp), None);
    }

    #[test]
    fn eli_mode_subcommands() {
        assert_eq!(EliMode::Chat.subcommand(), "chat");
        assert_eq!(EliMode::Gateway.subcommand(), "gateway");
        assert_eq!(EliMode::Chat.config_value(), "chat");
        assert_eq!(EliMode::Gateway.config_value(), "gateway");
    }
}
