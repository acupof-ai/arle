...
pub fn apply_runtime_flags(f: &TrainRuntimeFlags) {
    ...
    // Frozen-prompt-KV writeback now works under CP: the gen segment is CP-sharded
    // and attention seeds the ring accumulator from the full prompt prefix K/V
    // (off-tape, constant leaf). Cost scales with the supervised span, not the
