...
impl LinearWithLora {
    ...
    pub(crate) fn base_weight(&self) -> TensorId {
        self.weight
    }

    pub(crate) fn set_base_weight(&mut self, weight: TensorId) {
        self.weight = weight;
    }

    pub(crate) fn parameter_name_map(&self) -> HashMap<&'static str, TensorId> {
    ...
    pub(crate) fn adapter_ordered(&self) -> Vec<(&'static str, TensorId)> {
        match &self.lora {
            Some(lora) => vec![
                (lora.lora_a_name, lora.lora_a),
                (lora.lora_b_name, lora.lora_b),
    ...
    pub(crate) fn merged_tensor(&self, store: &mut TensorStore) -> Result<Tensor> {
        let shape = store
            .get(self.weight)
            .ok_or(AutogradError::InvalidTensorId(self.weight))?
            .shape
