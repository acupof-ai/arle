...
pub trait KvAllocator {
    ...
    /// retained by the prefix store.
    fn free_detached_pages(&mut self, pages: &[u32]);

    fn free_slot(&mut self, slot: usize);

    /// **Write-through evict-drop** — release one *middle* physical page of a
    /// live slot back to the free pool while the slot keeps decoding, leaving
    /// its logical length unchanged (the dropped page's KV is already mirrored
    /// to the tier by `KvTier::write_through`). The page is identified by its
    /// *logical* page index within the slot (`token_pos / page_size`); the
    /// implementation replaces that logical slot with an evicted sentinel so
    /// the remaining pages stay logically addressable, and returns the freed
    ...
    /// session grows: the *real* page free behind the write-through tiered KV
    /// model. No in-tree caller today — the `--kv-recall` driver was deleted
