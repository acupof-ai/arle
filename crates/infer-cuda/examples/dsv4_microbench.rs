...
// This example only does anything under `cuda`; a host-only build gets an
// explanatory no-op so `cargo test -p infer-cuda --no-default-features`
// (which compiles examples) stays clean — same pattern as `dsv4_parity`.
...
mod real {
    pub(super) fn run() -> anyhow::Result<()> {
        eprintln!("dsv4_microbench is a CUDA harness; rebuild with --features cuda.");
        Ok(())
    }
}
