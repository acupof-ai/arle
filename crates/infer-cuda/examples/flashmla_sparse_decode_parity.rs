...
mod real {
    ...
    pub(super) fn run(negative: bool) -> Result<()> {
        ...
        let ref_indices = oracle_indices(&tables, &selected);
        let (ref_out, ref_lse) = reference_attention(&records, &ref_indices, &sink, &splits_good);
        ...
        if !negative {
            ...
            println!("scheduler deterministic+bounded pass={sched_ok}");
            ensure!(
                pack_ok,
                "pack family: decoded pool differs from BF16 sources"
            );
            ensure!(idx_ok, "indices differ from semantic oracle");
        ...
        // indices/output/LSE: mask row 0's dominant selected entry.
        ...
        let bad_oracle = oracle_indices(&tables, &bad_sel);
        // The masked forward must still be self-consistent (it computes the
        // bad set correctly), so compare it to the bad oracle for a baseline,
        // then require it to have MOVED off the good reference.
        let (rel_bad, lse_bad) = {
            let (o, l) = reference_attention(&records, &bad_oracle, &sink, &splits_good);
            compare(&got_neg, &o, &l)
        };
    ...
    fn oracle_indices(tables: &[i32], selected: &[i32]) -> Vec<i32> {
