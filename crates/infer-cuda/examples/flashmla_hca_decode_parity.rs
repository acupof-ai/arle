...
mod real {
    ...
    pub(super) fn run(negative: bool) -> Result<()> {
        ...
        // Pack: flip the E8M0 scale byte for NoPE tile 0 of the first written
        // token. The scale region is at block_base + PAGE*(NoPE+RoPE bytes),
        // then row*8 + tile. Bit5 flips the e8m0 exponent by +-32, i.e.
        // scale x2^+-32 across the whole 64-lane tile, so the tile-relative
    ...
    fn check_pack(
        ...
        // Location of the worst NoPE tile, for the next GPU run: correct packs
        // report FP8-noise ratios <= ~0.062; an addressing/scale bug reports
        // ~0.5-1.5 and this names the exact (block,row,tile,e8m0).
