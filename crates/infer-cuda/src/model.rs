...
impl CudaModel {
    ...
    /// Session KV-recall eager decode forward (BF16 only).
    ///
    /// Same per-layer math as [`Self::forward_tokens`]'s decode branch, but
    /// attention reads a **restricted page table** (`recall_meta`, built from the
    ...
    ///
    /// Returns `(sampled_token, layer0_query)` where `layer0_query` is the
    /// post-RoPE layer-0 query as `[num_q_heads * head_dim]` f32 row-major.
    pub(crate) fn forward_decode_recall(
        &self,
        last_token: u32,
        pool: &mut PagedKVPool,
        ...
        position: u64,
            ...
            self.config.head_dim == 128,
            "recall decode only wires TileLang HD128 kernels, got head_dim={}",
            self.config.head_dim
        );
        ensure!(
            self.config.num_key_value_heads == 8,
            "recall decode only wires kv8 TileLang kernels, got kv_heads={}",
            self.config.num_key_value_heads
        );
