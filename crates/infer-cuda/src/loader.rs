...
impl SafetensorLoader {
    ...
    /// Vocab-row-sharded variant of [`Self::load_dsv4_global_matrix`] for the
    /// opt-in `ARLE_DSV4_LM_HEAD_SHARD=1` lever (#99): each TP rank uploads only
    /// its contiguous vocab slice `[rank*rows_per_rank, ..)`, zero-padded to one
    /// uniform 128-aligned `rows_per_rank` so NCCL all-gathers stay uniform and
    /// FP8 scale blocks stay rank-local. Because slices are contiguous and only
    /// the trailing rank(s) pad, a rank-order logits gather is vocab-ordered
    /// with the pad tail strictly past `vocab`. 128-aligned starts keep the
    /// BF16→FP8 host quantization byte-identical to the replicated load's
    /// blocks (block scales are 128-row-local).
    pub(crate) fn load_dsv4_global_matrix_vocab_sharded(
        &self,
        ctx: &DeviceContext,
        name: &str,
        tp: &TpConfig,
    ) -> Result<(DeviceMatrix, crate::dsv4::Dsv4LmHeadShard)> {
        ...
        let (vocab, cols) = (tensor.shape[0], tensor.shape[1]);
        let rows_per_rank = vocab.div_ceil(BLOCK).div_ceil(tp.world_size) * BLOCK;
        let start = tp.rank * rows_per_rank;
        ensure!(
            start < vocab,
            "{name}: vocab {vocab} too small to row-shard across {} ranks",
            tp.world_size
        );
        let local_rows = rows_per_rank.min(vocab - start);
        let shard = crate::dsv4::Dsv4LmHeadShard {
            vocab,
            rows_per_rank,
            local_rows,
        };
        let scale_pad_rows = rows_per_rank / BLOCK;

        let matrix = match tensor.dtype {
            Dtype::BF16 | Dtype::F32 => {
                let elem = if tensor.dtype == Dtype::BF16 { 2 } else { 4 };
                let bytes = tensor.bytes();
                ensure!(
                    bytes.len() == vocab * cols * elem,
                    "{name}: byte length {} != rows*cols*{elem}={}",
                    bytes.len(),
                    vocab * cols * elem
                );
                let slice = &bytes[start * cols * elem..(start + local_rows) * cols * elem];
                let (mut fp8, mut scales, _scale_rows, scale_cols) =
                    Self::quantize_to_dsv4_fp8_host(name, tensor.dtype, slice, local_rows, cols)?;
                // Pad rows are zero FP8 under a 1.0 (byte 127) scale → logits
                // exactly 0, and they are never read: argmax scans local_rows,
                // gathers strip everything past `vocab`.
                fp8.resize(rows_per_rank * cols, 0);
                scales.resize(scale_pad_rows * scale_cols, 127);
                DeviceMatrix::from_dsv4_fp8_block_scaled(
                    ctx,
                    &fp8,
                    &scales,
                ...
                .with_context(|| format!("upload vocab-sharded quantized DSv4 FP8 matrix {name}"))?
            }
            Dtype::F8_E4M3 => {
                let scale_name = name
                    .strip_suffix(".weight")
                    .map(|prefix| format!("{prefix}.scale"))
                    .ok_or_else(|| {
                        anyhow!("{name}: quantized DSv4 tensor must end with .weight")
                    })?;
                let scale = self.borrow_raw_tensor(&scale_name)?;
                ensure!(
                    scale.shape.len() == 2,
                ...
                let scale_e8m0 =
                    Self::dsv4_block_scale_e8m0(&scale_name, scale.dtype, scale.bytes())?;
                let (scale_rows, scale_cols) = (scale.shape[0], scale.shape[1]);
                ensure!(
                    scale_rows == vocab.div_ceil(BLOCK),
                    "{scale_name}: scale rows {scale_rows} != ceil({vocab}/{BLOCK})"
                );
                let bytes = tensor.bytes();
                ensure!(
                    bytes.len() == vocab * cols,
                    "{name}: FP8 byte length {} != rows*cols={}",
                    bytes.len(),
                    vocab * cols
                );
                let mut fp8 = bytes[start * cols..(start + local_rows) * cols].to_vec();
                fp8.resize(rows_per_rank * cols, 0);
                let scale_start = start / BLOCK;
                let scale_local = local_rows.div_ceil(BLOCK);
                let mut scales = scale_e8m0
                    [scale_start * scale_cols..(scale_start + scale_local) * scale_cols]
                    .to_vec();
                scales.resize(scale_pad_rows * scale_cols, 127);
                DeviceMatrix::from_dsv4_fp8_block_scaled(
                    ctx,
                ...
                .with_context(|| format!("upload vocab-sharded DSv4 FP8 matrix {name}"))?
            }
            other => bail!(
                "{name}: lm_head vocab shard supports BF16/F32/F8_E4M3, got {other:?} \
                 (unset ARLE_DSV4_LM_HEAD_SHARD for this checkpoint)"
            ),
        };
        Ok((matrix, shard))
    }
