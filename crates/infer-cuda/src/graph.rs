...
fn log_kernel_node_census(
    ...
    for (i, &node) in order.iter().enumerate() {
        ...
        if unsafe { cu::cuGraphKernelNodeGetParams(node, p.as_mut_ptr()) }
            .result()
            .is_err()
        {
            continue;
        ...
        // SAFETY: success above guarantees initialization.
        let p = unsafe { p.assume_init() };
        // CUDA 11 compat: cuFuncGetName/cuKernelGetName require CUDA 12.
        let name = if !p.func.is_null() {
            format!("<func {:p}>", p.func)
        } else {
            "<unnamed>".to_string()
        };
        log::info!(
