...
// Kept as the unfused primitive (A/B reference); current DSv4 decode uses
// `gen_mhc_params_pre_rms_norm_into`.
#[allow(dead_code)]
...
/// Fused decode-only [`gen_mhc_params_into`] + [`mhc_pre_rms_norm`]: one
/// kernel produces pre/post/comb into `scratch` AND the pre-mixed rms-normed
/// row (`pre` consumed from shared memory, never re-read from global).
/// post/comb stay in `scratch` for the later [`hc_post`].
#[allow(clippy::too_many_arguments)]
pub(crate) fn gen_mhc_params_pre_rms_norm_into(
    ctx: &DeviceContext,
    config: &DeepSeekV4Config,
    hc: &Dsv4HyperConnection,
    stream: &HiddenStates,
    ...
    let hc_mult = config.hc_mult;
        ...
        "DSv4 HC params+pre+norm stream dim {} != hidden_size {hidden_size} * hc_mult {hc_mult}",
        stream.hidden_dim
    );
    let mix_dim = (2 + hc_mult) * hc_mult;
    ensure!(
        ...
        "DSv4 HC params+pre+norm weight len {} != hidden {hidden_size}",
        norm_weight.len
    );
    ensure!(
        out.hidden_dim == hidden_size && out.seq_len == stream.seq_len,
        "DSv4 HC params+pre+norm out shape {}x{} != {hidden_size}x{}",
        out.hidden_dim,
        out.seq_len,
        stream.seq_len
    );
    ...
    // SAFETY: all buffers valid on ctx.stream; shapes checked above.
    unsafe {
        ffi::dsv4_mhc_params_pre_rms_norm_cuda(
            stream_ptr as *const ffi::Half,
            mixes_ptr as *const ffi::Half,
            base_ptr as *const ffi::Half,
            scale_ptr as *const ffi::Half,
