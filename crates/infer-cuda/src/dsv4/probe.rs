...
impl Dsv4ProbeCapture {
    pub(super) fn from_env(
        ...
        let host_bf16 = vec![half::bf16::default(); vocab * MAX_PROBE_ROWS];
    ...
    fn capture(
        ...
        // probe_flush ran, leaving stale positions from that forward.
        self.positions.clear();
        self.positions
            .extend_from_slice(&positions[positions.len() - rows..]);
        Ok(())
    }
    ...
    fn flush(&mut self, model: &Dsv4Model) -> Result<()> {
        ...
        for i in 0..self.lens_layers.len() {
            let layer_idx = self.lens_layers[i];
            self.layer_logits_to_host(model, layer_idx, rows)?;
    ...
    fn layer_logits_to_host(
        ...
        model
            .ctx
            .stream
            .memcpy_dtoh(&src, &mut self.host_bf16[..elems])
        ...
        for (i, &v) in self.host_bf16[..elems].iter().enumerate() {
...
fn compute_stats(logits: &[f32], reference: u32) -> (u32, f32, f32, bool) {
    let top1 = infer_plan::argmax_logit(logits);
    let max_logit = logits[top1 as usize];
