...
/// Re-materialize `layer_idx`'s quantized prefix rows into the shared bf16 work
/// buffers before the prefill prep kernel appends the new chunk. The work buffer is
/// overwritten every layer and prefix pages may arrive via radix attach / COW
/// detach / tier promote, so the quantized plane is the only durable source. K uses
/// the per-channel KIVI dequant, V the per-token sibling.
...
/// Quantize this forward's new bf16 rows work->pool for `layer_idx`, calibrating the
/// KIVI per-channel K scale table first if the layer's latch is still unset. K goes
/// through the per-channel quantize; V keeps per-(token, head) scales.
fn calibrate_and_quantize_new_rows(
    ...
    // Latch-once calibration is REQUIRED under chunked prefill: recalibrating on a
    // later chunk would rescale the table while earlier chunks' K bytes remain
    // quantized under the old scale, corrupting every prior row at decode.
...
fn run_quant_decode(
    ...
    // `choose_decode_num_splits` clamps the split count by workspace_bytes (>=1), so
    // the only unfittable case is a single split not fitting. Gating on 32 splits here
    // would falsely reject q40/q64 dense shapes at small num_slots.
...
fn prefill_attention(
    ...
    if quant {
        // Prefix rows must be back in the bf16 work buffer first: TileLang reads the
        // whole
        // [0, start_pos + seq_len) span from it via pool.k_ptr.
    ...
    if quant {
        // Must follow TileLang's read of the bf16 work buffer: calibrate (latch-once),
        // then persist this chunk's new rows into the quant planes.
...
fn decode_attention(
    ...
    if quant {
        ...
        // forward
        // arrives with start_pos==0 and zero-init static scales, and quantizing K
        // against a
        // zero table would write garbage for the whole request.
        calibrate_and_quantize_new_rows(ctx, layer_idx, pool, meta, num_kv_heads, head_dim)?;
...
fn run_tilelang_paged(
    ...
    // TileLang arg order is load-bearing: `num_pages` (arg 12) = pool capacity
    // (`pool.max_total_pages`), `total_pages` (arg 13) = page-table length
    // (`meta.num_pages`). Swapping them gives wrong pool strides and an OOB
    // kv_indices walk that hangs the kernel (Xid 43).
    let phase = if decode {
