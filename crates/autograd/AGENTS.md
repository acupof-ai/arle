...
│   ├── lr_schedule.rs  — `LrSchedule` trait + `CosineWithWarmup`. Pure
│   │                     step→f32 function (no persisted state). The earlier
│   │                     `ConstantLr` / `LinearWarmup` / `parse_lr_schedule`
│   │                     were removed in `47bedcd6d`; CLI schedules are
│   │                     `cli::args::LrScheduleArg`.
...
- **HF Trainer scheduler defaults are not neutral.** Linear LR decay over 500 steps changes
  ...
  reads framework defaults as quality.
...
- **Cross-engine validation against PyTorch reference catches silent serving-side corruption.**
  ARLE-only smoke tests can pass for weeks during a GDR prefill bug because they have no PyTorch
