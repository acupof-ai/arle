...
use autograd::{
    ...
    ops::gather_last_dim, ops::log_softmax, ops::matmul, ops::mean, ops::mul, ops::mul_scalar,
    ops::reshape, ops::rmsnorm, ops::rope, ops::sigmoid, ops::silu, ops::slice, ops::sum,
...
fn metal_adamw_step_stays_device_resident() -> Result<()> {
    ...
    for step in 0..STEPS {
        ...
        host_opt
            .step(&[host_param], &mut host_store)
            .expect("host adamw step");
    }
    ...
    for step in 0..STEPS {
        ...
        reset_eval_count();
        dev_opt
            .step(&[dev_param], &mut dev_store)
            .expect("dev adamw step");
...
fn metal_adamw_step_batches_eval_across_params() -> Result<()> {
    ...
    let measure = |n_params: usize| -> Result<u64> {
        ...
        reset_eval_count();
        opt.step(&param_ids, &mut store).expect("adamw step");
