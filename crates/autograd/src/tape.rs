...
pub enum SavedContext {
    ...
    LinearAttentionCtx {
        ...
        /// Some = the FlashQLA route ran; the pre-norm GDN output its backward
        /// differentiates. The runtime flag can flip between calls, so presence here
        /// is what the backward reads, never the flag.
    ...
    RingAttentionCtx {
        ...
        /// Frozen-prompt-KV prefix block `(k, v, gen_start)`, or `None` for plain CP.
        ...
        /// owner (every rank holds the full prefix). `gen_start` reconstructs the
        /// prefix positions `0..gen_start` (always contiguous from 0).
