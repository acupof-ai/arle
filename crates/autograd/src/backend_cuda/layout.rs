...
#[cfg(not(feature = "no-cuda"))]
#[cfg(not(feature = "no-cuda"))]
pub(super) fn cuda_permute_seq_blocks_device(
    backend: &CudaBackend,
    x: &DeviceHandle,
    batch: usize,
    num_blocks: usize,
    ...
    // A block is a contiguous element range in [batch, seq, dim] row-major, so the
    // whole permutation is one D2D copy per block — no per-block tensor, and the
    // adjoint is the inverse permutation rather than a full-input zero buffer.
    macro_rules! permute {
        ($elem:ty, $src:expr, $wrap:expr) => {{
            if $src.len() != total {
                return Err(AutogradError::DataLengthMismatch {
                    len: $src.len(),
                    shape: vec![batch, num_blocks, block_elems],
            ...
            let mut dst = unsafe { backend.stream.alloc::<$elem>(total) }
                .map_err(|_| AutogradError::TapeInvariant("cuda alloc failed (permute_seq)"))?;
            for b in 0..batch {
                for (i, &from) in perm.iter().enumerate() {
                    if from >= num_blocks {
                        return Err(AutogradError::TapeInvariant(
                            "permute_seq_blocks: permutation index out of range",
                        ));
                    }
                    ...
                    backend
                        .stream
                        .memcpy_dtod(
                            &$src.slice(s..s + block_elems),
                            &mut dst.slice_mut(d..d + block_elems),
                        )
                        .map_err(|_| {
                            AutogradError::TapeInvariant("permute_seq block copy failed")
                        })?;
                }
            }
            Ok($wrap(dst))
    ...
    match x {
        ...
        _ => {
            ...
            permute!(f32, src, |d| DeviceHandle::Cuda(CudaStorage::new(d)))
        }
    }
}
