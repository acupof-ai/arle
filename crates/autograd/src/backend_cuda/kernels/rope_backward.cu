// RoPE backward: gx = rope_forward(gy, cos, -sin). The sin sign flip is done
// inline per-element instead of a separate negate kernel.
...
extern "C" __global__ void rope_backward_f32(
    ...
    int seq,
    int head_dim,
    int rot_half  // grads of the non-rotary tail pass through.
) {
