...
pub(super) fn cuda_ep_exchange_rows_device(
    backend: &CudaBackend,
    x: &DeviceHandle,
    dim: usize,
    send_counts: &[usize],
    ...
    let src = backend.cuda_slice(x, "ep_exchange_rows")?;
    ...
    #[cfg(all(feature = "nccl", not(feature = "no-cuda")))]
    {
        ...
        if send_counts[rank] > 0 {
            ...
            let mut dst = out.slice_mut(recv_offs[rank]..recv_offs[rank] + recv_counts[rank] * dim);
            backend
                .stream
        ...
        }
        {
            ...
            let (dst_ptr, _dg) = out.device_ptr_mut(&backend.stream);
            nccl.group_start()
            ...
            for j in 0..world {
                ...
                unsafe {
                    if send_counts[j] > 0 {
                        ...
                        .map_err(|_| AutogradError::TapeInvariant("ep_exchange send failed"))?;
                    }
                    if recv_counts[j] > 0 {
                        nccl.recv(
                        ...
                        .map_err(|_| AutogradError::TapeInvariant("ep_exchange recv failed"))?;
                    }
                }
            }
            nccl.group_end()
