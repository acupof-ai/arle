...
mod tests {
    ...
    #[test]
    fn parses_deepseek_ocr_config() {
        let cfg = DeepseekOcrConfig::from_json_str(SAMPLE).unwrap();
        assert_eq!(cfg.text.hidden_size, 1280);
        ...
        assert_eq!(cfg.text.shared_expert_intermediate_size(), 1792);
        assert!(!cfg.text.is_moe_layer(0));
        assert!(cfg.text.is_moe_layer(1));
        assert!(cfg.text.is_moe_layer(11));
        assert_eq!(cfg.vision.hidden_size, 1024);
        assert_eq!(cfg.vision.num_hidden_layers, 24);
        assert_eq!(cfg.vision.num_attention_heads, 16);
        assert_eq!(cfg.sam.width, 768);
        assert_eq!(cfg.sam.global_attn_indexes, vec![2, 5, 8, 11]);
        assert_eq!(cfg.projector.input_dim, 2048);
        assert_eq!(cfg.projector.n_embed, 1280);
        assert!(cfg.is_mxfp8());
        assert_eq!(cfg.tile_tag, "2D");
        assert_eq!(cfg.global_view_pos, "head");
    }
}
