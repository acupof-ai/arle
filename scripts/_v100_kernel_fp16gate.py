...
sm_70 (Volta) variant under test: I/O tensors stay bf16 (runtime ABI), but the
GEMM operands are fed in fp16 on sm<80 so the gemms route to TileLang's stock
Volta tensor-core path (GemmMMASm70 / mma.sync) instead of the scalar cuda.fma
fallback. The fma fallback's per-output _linear_fragment layout diverges between
scores(64x64) and acc_o(64x128) → LayoutInference m_prev/scale_i conflict. The
MMA path reconciles both layouts exactly like sm_80+, so the conflict vanishes
and we get real tensor cores. sm>=80 keeps the byte-identical bf16 path.
    ...
    def kernel(
        ...
        with T.Kernel(
            ...
            for kn in T.Pipelined(T.ceildiv(kv_visible_end, BLOCK_N), num_stages=NUM_STAGES):
                ...
                # on the Volta path (p/p_gemm conflict), but a fragment->shared
                ...
                if gemm_dtype != dtype:
                    ...
                    p_gemm = T.alloc_fragment((BLOCK_M, BLOCK_N), gemm_dtype)
                    T.copy(p, p_gemm)
                    T.gemm(p_gemm, v_tile, acc_o, policy=T.GemmWarpPolicy.FullRow)
