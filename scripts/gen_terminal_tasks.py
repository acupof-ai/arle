...
def main() -> int:
    ...
    ap.add_argument("--min-complexity", type=int, default=1, choices=[1, 2, 3])
    ...
    pool = [b for b in BUILDERS
            if args.min_complexity <= b(random.Random(0))["complexity"] <= args.max_complexity
            and (not want or b(random.Random(0))["domain"] in want)]
    if not pool:
        sys.exit(f"no builders match domains={want} "
                 f"complexity=[{args.min_complexity},{args.max_complexity}]")
