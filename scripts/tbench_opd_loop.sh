...
# SIGKILL — serve ignores SIGTERM.
kill_serve(){ for p in $(pgrep -x arle 2>/dev/null); do tr '\0' ' ' < /proc/$p/cmdline 2>/dev/null | grep -qE -- "--port $PORT([[:space:]]|$)" && kill -9 "$p" 2>/dev/null; done; sleep 5; }
