...
# Bind 0 in the server and read the assigned port back; $3 names the variable
# to receive it. A fixed port collided between overlapping hook runs on a shared
# box and surfaced as a test failure rather than a bind conflict.
start() {
    local pf="$TMP/port.$(basename "$2")"; rm -f "$pf"
    FAILMODE="$1" python3 "$2" 0 "$pf" >/dev/null 2>&1 & SRV_PID=$!
    local i
    for i in $(seq 1 100); do [ -s "$pf" ] && break; sleep 0.05; done
    eval "$3=\$(cat "$pf")"
}
stop() { kill "$SRV_PID" 2>/dev/null || true; wait "$SRV_PID" 2>/dev/null || true; }
# Closed listener = dead serve: bind 0, take the port, release it.
...
stop
expect 2 "dead serve is infra, not regression" python3 "$ROOT/scripts/sampling_gate.py" "$(dead_port)"
...
stop
expect 2 "longctx dead serve is infra" "${LONG[@]}" --left-url "http://127.0.0.1:$(dead_port)"
