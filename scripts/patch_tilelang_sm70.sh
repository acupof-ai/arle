...
# V100 builds work today. The patch carries a cuda.fma T.gemm fallback for
# dtype/layout combinations unsupported by SM70 MMA.
#
# REMOVE WHEN: PR https://github.com/tile-ai/tilelang/pull/2339 merges and a
...
if [ ! -d "$TILELANG_DIR/src/cuda/op" ]; then
  echo "error: $TILELANG_DIR does not look like a TileLang checkout" >&2
  echo "  (expected src/cuda/op/gemm.cc)" >&2
  exit 1
fi
...
if grep -q 'kCudaFMA = "cuda.fma"' src/cuda/op/gemm.cc 2>/dev/null &&
  grep -q "GEMM_INST_FMA" tilelang/cuda/op/gemm/gemm_fma.py 2>/dev/null; then
  echo "patch already applied to $TILELANG_DIR (GemmFMA fallback found); skipping"
  exit 0
fi
