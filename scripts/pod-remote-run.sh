...
dirty_digest() { git -C "$TREE" status --porcelain=v1 -z -- . ':(exclude).arle-source-receipt' | sha256sum | cut -d' ' -f1; }
...
case "${1:-}" in
  status|log|kill)
    action="$1"; label="${2:?missing label}"; found=0
    while IFS= read -r dir; do
      [ -n "$dir" ] || continue; found=1
      if [ "$action" = log ]; then cat "$dir/log" 2>/dev/null; continue; fi
      if valid_process "$dir/process"; then
        if [ "$action" = kill ]; then pgid="$(field "$dir/process" pgid)"; kill -- "-$pgid" && echo "killed $(basename "$dir") pgid=$pgid"
        else echo "$(basename "$dir"): RUNNING pid=$(field "$dir/process" pid)"; fi
      elif [ "$action" = kill ]; then
        echo "refuse stale or mismatched process: $(basename "$dir")" >&2; exit 1
      else
  ...
  run)
    ...
    if [ ! -f "$BUILD_RECEIPT" ] || [ "$(field "$BUILD_RECEIPT" schema)" != arle-build-v1 ] || [ "$(field "$BUILD_RECEIPT" exit)" != 0 ]; then
      ...
      elif [ ! -f "$TREE/.arle-source-receipt" ] || [ "$(field "$TREE/.arle-source-receipt" head)" != "$source_head" ] || [ "$(field "$TREE/.arle-source-receipt" digest)" != "$source_digest" ] || [ "$(git -C "$TREE" rev-parse HEAD)" != "$source_head" ] || [ "$(dirty_digest)" != "$source_digest" ]; then echo "source changed since build"
      else
