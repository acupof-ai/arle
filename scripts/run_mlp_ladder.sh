#!/usr/bin/env bash
# Usage: run_mlp_ladder.sh <label> <gpu> <seq> <serve_port>
# MLP/attention sequence chunking is baked in at OPD_SEQ_CHUNK=4096.
set -euo pipefail
LABEL="${1:?label}"; GPU="${2:?gpu}"; SEQ="${3:?seq}"; PORT="${4:?serve_port}"
...
LOG="$OUT/run.log"
{
  echo "=== $LABEL gpu=$GPU seq=$SEQ binary_sha=$(sha256sum "$BIN" | cut -d' ' -f1) start=$(date -u +%FT%TZ) ==="
  rc=0
  CUDA_VISIBLE_DEVICES="$GPU" INFER_CUDA_DEVICE=0 \
  ARLE_OPD_VRAM_TRACE=1 ARLE_AOPD_PROFILE=1 RUST_LOG=info \
    "$BIN" train agent-opd \
