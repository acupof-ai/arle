...
LOG="$OUT/run.log"
{
  echo "=== $LABEL gpu=$GPU binary_sha=$(sha256sum "$BIN" | cut -d' ' -f1) start=$(date -u +%FT%TZ) ==="
  rc=0
  CUDA_VISIBLE_DEVICES="$GPU" INFER_CUDA_DEVICE=0 \
  ARLE_OPD_VRAM_TRACE=1 ARLE_AOPD_PROFILE=1 ARLE_OPD_OP_MEM_CHECKPOINT_FN=60 RUST_LOG=info \
    "$BIN" train agent-opd \
