#!/usr/bin/env bash
# C2 clamp test (Qwen3.5/3.6 MoE lane, TP=2): oversized --num-slots at a normal
# 128K context (total_pages 8192 × page_size 16) drives requested ≫ affordable,
...
setsid bash -lc '
  ...
  exec /data01/build/arle/target/release-fast/arle serve --backend cuda \
    ...
    --num-slots 1024 --total-pages 8192 --page-size 16 --kv-cache-dtype auto
' < /dev/null > "$LOG" 2>&1 &
