"""TileLang batch decode HD256 paged attention with FP8 E4M3 KV.

Sister to ``batch_decode_paged_hd256.py`` (BF16 KV path). Deltas:

  * ``K_pool`` / ``V_pool`` dtype: ``"bfloat16"`` → ``"float8_e4m3fn"``.
  * Adds ``K_scales`` / ``V_scales`` tensors of shape
    ``(num_pages, PAGE_SIZE, num_kv_heads)`` in fp32 — per-token per-kv-head
    scale factors. Layout matches the PagedKVPool ``[max_pages * page_size,
    num_kv_heads]`` row-major allocation.
  * Inline dequant: ``T.cast(T.cast(fp8, "float32") * scale, "bfloat16")``
    before the value lands in ``k_tile`` / ``v_tile`` (shared BF16), so
    ``T.gemm(q_tile, k_tile, ...)`` sees matching BF16 dtypes.
  * Drops the sm<80 ``gemm_dtype``/``to_gemm()`` complexity — FP8 pools
    require sm≥80 (Ampere+), so allow_sm70 = false always.
  * ABI: ``paged_attn_fp8_v1`` (20-arg, adds k_scales/v_scales vs 18-arg BF16).

SUPPORTED_HEADS mirrors the BF16 HD256 decode config.
"""
...
def _make_kernel(num_q_heads: int, num_kv_heads: int):
    ...
    def kernel(
        ...
        # Grid: (1, num_q_heads, batch_size) — decode has qlen == 1.
        with T.Kernel(1, num_q_heads, batch_size, threads=NUM_THREADS) as (bx, by, bz):
            ...
            for kn in T.Pipelined(T.ceildiv(kv_total_len, BLOCK_N), num_stages=NUM_STAGES):
                ...
                for j, d in T.Parallel(BLOCK_N, HEAD_DIM):
                    ...
                    # FP8 → f32 → * scale → BF16 (inline dequant).
                    ...
                    k_tile[j, d] = T.if_then_else(abs_col < kv_total_len, k_bf16, T.cast(0, out_dtype))
                    ...
                    v_tile[j, d] = T.if_then_else(abs_col < kv_total_len, v_bf16, T.cast(0, out_dtype))
                ...
                for i, j in T.Parallel(BLOCK_M, BLOCK_N):
                    ...
                    scores[i, j] = T.if_then_else(
                        in_bounds, scores[i, j] * sm_scale, -T.infinity(accum_dtype)
                    )
            ...
            for i, d in T.Parallel(BLOCK_M, HEAD_DIM):
                if i == 0:
                    Output[bz, by * HEAD_DIM + d] = T.cast(acc_o[i, d] / l_i[i], out_dtype)

    kernel.__name__ = f"batch_decode_paged_hd256_fp8_q{num_q_heads}_kv{num_kv_heads}_run"
    return kernel
