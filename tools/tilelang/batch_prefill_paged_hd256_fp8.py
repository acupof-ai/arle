"""TileLang batch prefill HD256 paged attention with FP8 E4M3 KV.

Sister to ``batch_prefill_paged_hd256.py`` (BF16 KV path). Deltas:
  ...
  * Adds ``K_scales`` / ``V_scales`` tensors of shape
    ...
    scale factors matching the PagedKVPool layout.
  * Inline dequant in the KV load loop before values land in shared BF16
    ``k_tile`` / ``v_tile``.
  * Drops the sm<80 ``gemm_dtype``/``to_gemm()`` complexity — FP8 pools
    require sm≥80 (Ampere+), so allow_sm70 = false always.
  * BLOCK_N = 32 (= PAGE_SIZE * 2), same as the BF16 prefill twin to keep
    shared memory under the sm_89 100 KB cap.
  * ABI: ``paged_attn_fp8_v1`` (20-arg, adds k_scales/v_scales).

For chunked prefill (start_pos > 0): prefix is already in ``k_data[layer]``
as FP8 from prior steps; the Rust host quantizes the new chunk from the BF16
work buffer → FP8 data buffer BEFORE calling this kernel, so ALL tokens
(prefix + new chunk) are in the FP8 data pool for this kernel to read.
"""
...
def _make_kernel(num_q_heads: int, num_kv_heads: int):
    ...
    kernel.__name__ = f"batch_prefill_paged_hd256_fp8_q{num_q_heads}_kv{num_kv_heads}_run"
    return kernel
