...
- **Verification harness — Metal gate, CI needle gate, concurrent arm, bench_compare fix.** `needle_gate.py --check` exits 0/1 without a baseline; `lever_gate.sh GATE_PROFILE=metal` boots a Metal serve; temp + concurrent needle arms integrated (parallel, skip with env); `bench_compare.py` rewritten for `arle.bench_throughput.v1` format with COLLAPSE detection and cross-workload refusal; Metal CI now runs the needle ladder on Qwen3.5-0.8B. See `docs/experience/wins/2026-08-23-verification-harness-metal-gate.md`.
