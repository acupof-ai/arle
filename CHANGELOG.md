...
- **DSv4 c=1 decode graph — default flip: armed by default (`1a48d179f`); `ARLE_DSV4_DECODE_GRAPH=0` selects the eager arm.** TP=4 on H20, 32k agent prompts: NVFP4 c=1 decode 35.6 → 43.2 tok/s (+21.3%), ITL p50 24.5 → 22.3 ms, p99 122.6 → 44.5 ms (−63.7%); FP8 c=1 52.4 → 59.5 tok/s, ITL p50 18.5 → 16.3 ms. c=8/16 unchanged (gate is c=1-only), DSpark unchanged with 0 captures, MMLU 171/200 in both arms with 0 per-item diffs. Capture audit: 0 host nodes; the alloc-node count was 86, not the 0 first reported (a grep that could not match the log text) — corrected in the entry, numbers unaffected. Eleven replay defects fixed on the way, including a prefix-cache restore that left a stale graph armed on a re-pointed slot. See `docs/experience/wins/2026-08-23-dsv4-c1-decode-graph.md`.
