...
- **FEAT — DP coordinator: least-in-flight multi-group routing** (2026-08-17; `806c4268a`, [wins](docs/experience/wins/2026-08-17-dp-coordinator.md)). `INFER_DP_SIZE=M` spawns M independent TP groups on disjoint GPUs; a `DpCoordinator` wrapping `Vec<Arc<CoordinatorHandle>>` routes each request to the least-in-flight group via `Deref`-based selection. Handler bodies stay unchanged — Deref coercion fixes the target group per call site. Per-group NCCL unique ID + relay port; GPU assignment `group × world_size + rank`. Pod gate pending.

- **PERF — TP/CP NCCL collectives onto comm_stream** (2026-08-17; `a59c6c661`, [wins](docs/experience/wins/2026-08-17-collectives-to-comm-stream.md)). All NCCL collectives (all-gather, CP send/recv/broadcast) moved from the compute stream to `comm_stream`, bracketed by `comm_waits_for_compute` / `compute_waits_for_comm` fences. The dsv4 prefill shared-expert overlap is preserved by keeping the moe all-reduce on the compute stream. One-shot all-reduce stays on compute (small-message fast path). Enables the T3.2 CP-decode merge path to overlap compute. Pod gate pending.

- **PERF — DeepEP host stalls → on-device event ordering** (2026-08-17; `142b959d4`, [wins](docs/experience/wins/2026-08-17-deepep-host-stalls-event-ordering.md)). The DeepEP C++ wrapper had event-based stream ordering (`cudaEventRecord` + `cudaStreamWaitEvent`) but every Rust call site passed `compute_stream=0`, silently falling back to `cudaStreamSynchronize` — a per-MoE-layer host block. Passing the real compute stream activates the event path; intranode dispatch gains event support it lacked. The `recv_topk_idx` D2H→host-i64→i32→H2D roundtrip replaced with an on-device cast kernel. Smoke test PASS on DSv4 MoE TP=4; nsys A/B pending.

- **FEAT (accept) — 35B A3B AutoRound W4A16 runs on V100 (sm_70) via ARLE** (2026-08-17; [wins](docs/experience/wins/2026-08-17-autoround-w4a16-v100.md)). The Qwen3.6-35B-A3B hybrid model (GDR linear attention + gated full attention + 256 experts) now produces coherent output on a single V100-32GB via ARLE, using the `LeaderboardModel1/Qwen3.6-35B-A3B-autoround-W4A16` checkpoint. Three blockers found and fixed in the checkpoint-prep path: (1) AutoRound quantizes every linear layer including GDR and shared_expert — a loader that only dequantizes attention projections leaves GDR weights as meta tensors → all-NaN from layer 0; (2) AutoRound exports q_proj in the correct interleaved layout (the old llm-compressor checkpoint needed a concatenated→interleaved permutation; applying it to AutoRound corrupts the weights); (3) ARLE's GPTQ detection is gated on `quantization_config` in `config.json` — AutoRound omits the field, so detection was silently disabled. ARLE's native GPTQ W4A16 path produces NaN for non-expert weights (root cause unconfirmed; suspect small-N GEMV edge case); workaround: dequant all non-expert GPTQ weights to BF16, keep experts in GPTQ. Load 122 s, 30.2 GB VRAM, coherent output ("The capital of France is Paris."). Decode **28 tok/s**, TTFT 0.53 s (40× the 0.7 tok/s PyTorch fallback).
