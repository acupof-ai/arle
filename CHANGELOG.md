...
- **Per-request prefix-cache reuse is reported as OpenAI `cached_tokens` (#409).** The count is captured where the prefix is attached, so it is the restored length itself rather than a value read back from a cursor that later moves, and it travels as an `Option` from the engine's request state through the completed request, the local and cross-process relays, and into `Usage`. An absent value stays absent: the relay field carries `serde(default)`, a delta without the field deserializes to unreported rather than zero, and the usage object omits `prompt_tokens_details` entirely instead of emitting a hard-coded zero the way it did before. A measured zero-reuse request still reports `0`, which is a different statement from silence. The lane was bounced once for a defect this repository has now seen in several shapes: the n>1 chat path summed the per-choice cached counts while the prompt itself was counted once, so four choices over a fully reused 100-token prompt reported 400 cached against 100 prompt — a breakdown exceeding the total it breaks down. Cached tokens are prompt-scoped and every choice attaches to the same prefix, so the combiner takes the maximum; an unreported choice never masks a measured one, and a genuine disagreement keeps the largest real value rather than averaging it away. The regression test was confirmed red against the summing version. Entry: [docs/experience/wins/2026-09-12-cached-tokens-per-request.md](docs/experience/wins/2026-09-12-cached-tokens-per-request.md).
- **`lane.sh` resolved its own root from the script's location, so running it inside a lane built a second target directory (#411).** Every lane worktree carries a tracked copy of the script, so `BASH_SOURCE` there points at the lane rather than the main checkout. Invoked from inside a lane, `new` therefore doubled the lanes directory to `arle-lanes/arle-lanes`, created the worktree in that misplaced location, and wrote a `.cargo/config.toml` whose `target-dir` pointed at the invoking lane's own `target/` — a second full build directory instead of the shared one the lane scheme exists to guarantee. This was not hypothetical: the artifact was on disk, written 2026-09-11, naming a lane that no longer exists. The same wrong root reached `pr` and `rm`, which fail loudly on it, and `share_target`, which is reached only through `new` and so inherited it silently; `doctor` was unaffected. The root now comes from git's shared common directory, which resolves to the main checkout from the main tree and from any linked worktree alike. The regression test constructs the real failure — a lane, a copied script, a `new` run from inside it — and asserts the doubled directory is absent and exactly one cargo config exists, pointing at the main checkout; it was confirmed red against the previous derivation, where those four assertions fail and the five pre-existing cases still pass. Bench-entry exemption: development tooling only.
- **Three stale baseline rows now say so, and two inherited citations are repaired (#410).** The 8×H20 DSpark row from 2026-08-14 is marked stale at c=8/16 because three multi-GPU default flips landed after it — TP decode graph, TP batched decode, and FlashQLA chunked attention at attn_tp=8 — and its c=1 point also predates the batched spec-verify reroute; the 4×H20 c=1-graph row keeps its decode columns and loses only its TTFT column, which predates the chunked-GDN prefill default at attn_tp=2/4. No number was recomputed or deleted: each callout names the successor run, and the successor for the 8-card row is queued as P15 behind the P3 verify grid. The V100 DSpark row is the case that could not be handled the same way — the lockstep stall it records was fixed, but the sm_70 drafter defect behind the arm was closed wont-fix on 2026-07-31 because V100 is a verification lane rather than a deployment target, so the row is annotated as a pre-fix record with no successor by decision, and the queue's exclusion list says not to schedule one without reopening that issue. The Qwen3.8 support-matrix row now carries the 2026-08-22 ITL set with its source named, and states why the advantage moved without the treatment changing: the FP8 comparison baseline improved 14.6%, while the NVFP4 checkpoint did not change. Its end-to-end and 8-token figures are kept and explicitly dated 2026-08-20 as not remeasured. Two line-number citations into another document are replaced with section references, and one citation that cannot be resolved is labelled unresolvable rather than filled with a substitute number. Bench-entry exemption: documentation only.
- **Around ninety Rust test functions compile and never run (#408).** Every `#[test]` in the tree was compared against the actual `cargo test` invocations in the workflows and the pre-push hook, feature sets included. No invocation anywhere names `cuda-kernels` or `infer-cuda`, so nineteen host-only test functions in those crates — quantization routing, tensor-parallel budget, weight-format validation, ring math — have never executed once, on any machine, despite needing no GPU. A second mechanism kills a whole layer at a stroke: `cargo test -p autograd --lib` excludes the `tests/` target, so roughly fifty-two Metal-gated functions are not even compiled, and the flag is justified by a comment in the workflow claiming those crates have no `tests/` directory and that their tests are CUDA cases — both statements are false. A third turns a missing fixture into a pass: a loader smoke test returns `Ok(())` when the model directory is absent and its header explains that it avoids `#[ignore]` so the skip stays visible, which it is, as `ok`. Clippy compiles all of it, so the crates read as covered. Nothing is fixed yet and that is deliberate: a test that has never executed is not known to pass, so wiring it into CI before running it would make a pre-existing failure look like a wiring error. The runs are queued behind the pod being free. Entry: [docs/experience/errors/2026-09-12-rust-tests-never-executed.md](docs/experience/errors/2026-09-12-rust-tests-never-executed.md). Bench-entry exemption: documentation only.

- **The needle gate now says which channel answered, without changing what it accepts (#407).** A thinking model can return the needle in `reasoning_content` while `content` is empty; the greedy arm reads content only and scores that a miss, while the temp arm concatenates both and scores it a pass, so the two arms disagreed silently. Rather than widen the criterion to match the permissive arm, each run now reports where the needle was found and the summary carries a reasoning-only count, leaving the verdict untouched. The investigation found a fourth explanation that outranks all three of the obvious ones: the gate sends `NEEDLE_MAX_TOKENS=16`, and against a thinking model the budget is consumed before an answer is emitted. That also bounds what the new marker can mean — `split_reasoning` returns an unclosed think block as content (`infer-server/src/schema.rs:1169`), so a plain cutoff cannot produce a reasoning-only result; it takes a closed block followed by no answer. Widening would therefore have made the gate pass a configuration in which a real client received nothing, which is the expensive direction for the one gate that licenses correctness. The position, its reasoning and the decision table are recorded in `docs/plans/2026-09-12-reasoning-content-criterion.md`, and the run that settles it is queued as P14 against a real thinking model. A new shell test pins all four location shapes including the divergence, and the enriched summary line stays readable by the existing parser. Bench-entry exemption: gate harness and documentation only.

- **A support level stated in two documents diverged, licensing a serve that does not exist (#406).** `docs/quantization.md` listed GGUF Q3_K through Q6_K as production on CUDA and Metal while `docs/support-matrix.md`, corrected in #390, said GGUF is not served on either. The matrix was right: `infer-cuda` and `infer-metal` declare no `infer-gguf` dependency and have no GGUF loader branch, and the K-quant GEMV kernels' only callers outside the kernel tree are the HIP path; the kernels compile without the serve path existing. Q3_K has no host launcher on any backend at all. The same page understated the opposite way, calling DSv4 FP8 and FP4 block-scaled work in progress blocked on kernels that are in fact dispatched on the serving path, and it called the FP8 KV cache production where the matrix says opt-in. All corrected, with the dispatch sites cited at file and line. The divergence happened because the fact lives in two places and only one was fixed, so the page now names the support matrix canonical on disagreement and its status words were aligned to the matrix's five: production, opt-in, experimental, deferred, not implemented. Two cargo test targets that no longer exist were removed from the harness table, replaced by the parity example that covers the same ground; the cross-reference section, whose seven entries and five commit hashes had all been deleted, was removed rather than repopulated with plausible substitutes. Bench-entry exemption: documentation only.

- **The concurrent needle gate recorded a dead server as rows missing their needle (#405).** `needle_concurrent.py` added every failed request to the same counter as a genuine cross-row mix-up, so a run against a server that was not listening printed `CONCURRENT_NEEDLE FAIL total_miss=N` — and that summary line is what gets quoted into experience entries. A reader weeks later would have attributed a serve outage to batched decode losing cross-row state. The gate still failed closed, so nothing passed that should not have; the damage was to the record. Following the convention set in #400, a request error now exits 2 and is checked before the miss verdict, a cross-row or retrieval miss stays 1, and both the per-round line and the summary report the error count separately from the miss count with errored rows excluded from the latter. What counts as a miss is unchanged, and `lever_gate.sh` is unaffected because it treats any non-zero as a failure. Nothing in the repository parses the summary line, so widening it is safe. A new shell test drives the real script against in-process servers and asserts all four outcomes — clean pass, wrong secret, malformed transcript, dead port — checking each summary line as well as each exit code; it is registered in the pre-push list. Bench-entry exemption: gate harness scripts only.

- **The DSv4 architecture document described a module tree that no longer exists (#403).** Its named stable anchors — `try_flashmla_decode_attention`, `try_flashmla_prefill_attention`, `kv_budget_num_slots` — return zero hits in the tree, and the flat `dsv4.rs` / `attention.rs` / `spec_decode.rs` / `moe.rs` layout it walked through has since split into directories, so a reader following it reached files that had moved and symbols that had been renamed. Twenty-two claims were corrected against the current tree, each to a symbol at a named file and line, including the fourteen rows of the symbol index; a commit hash that no longer resolves was replaced with the stable issue number, a CLI flag documented in the present tense was found to have been removed, and two reference links pointing at deleted experience entries were retargeted to the surviving ones. The document's self-labelled 2026-06-29 line-number snapshots were left alone, since a dated snapshot of a line number is honest about what it is, and so was the §8 roadmap row, since a plan that has not happened is not a false claim. Two `d_qk` values could not be established from the upstream spec and are recorded as unverified by this pass rather than silently blessed. Bench-entry exemption: documentation only.

- **A pod-flow test asserted that a busy GPU is refused while the GPU was in fact granted (#402).** Nine sites in `scripts/tests/test_pod_flow.sh` were written as `[ "$rc" -ne 0 ] && <assertion>`. Under `set -e` that compound aborts nothing: it returns non-zero harmlessly and skips the assertion in exactly the case the test exists to catch, when the guarded command unexpectedly succeeds. Converting all nine to independent bracketed assertions exposed a defect the form had been hiding. The busy-GPU site built its foreign claim with the live shell's own pid, but this test host has no `/proc`, so `pick-gpu.sh` classified the claim stale, reaped it, granted the set, and returned 0 — the assertion never ran and the suite stayed green while the site reported the opposite of what happened. The fixture now supplies a synthetic `PROC_ROOT`, which the production script already honours everywhere it reads process state, so the state is constructed identically on a host with or without `/proc`; a reverse control differing only in whether the claimed pid exists proves the check keys on liveness rather than on the claim file being present. What each site asserts is unchanged. Nineteen mutations were each shown red at their own site, and an inert `kill -0 $$` self-check that could never fail was removed. Bench-entry exemption: test harness only. The block dates to 2026-07-19 (`e5d9bc1b4`), so it was green and meaningless for 55 days; entry: [docs/experience/errors/2026-09-12-refusal-test-hostile-state-fixture-platform.md](docs/experience/errors/2026-09-12-refusal-test-hostile-state-fixture-platform.md).
