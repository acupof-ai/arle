...
- **FIX (pending-remote) — reshape/rmsnorm backward no longer drops to a host round-trip** (2026-08-06; `7da312d0d`, [win](docs/experience/wins/2026-08-06-reshape-rmsnorm-backward-host-fallback-heal.md)). Probe C flamegraph of the 80K OPD backward put **21% of on-CPU self-time in `reshape`/`rmsnorm` backward → libc `memcpy`**. Root cause: both gate device-vs-host on operand residency but, unlike `matmul_backward`, never `ensure_device` first — so one `Dirty::Host` upstream grad drops them to the host fallback (`tensor_host` full-`Vec` clone + DtoH readback + downstream HtoD re-upload), and the demotion cascades through every reshape/rmsnorm below it. Fix mirrors `matmul_backward`'s heal (`ensure_device` on upstream/x/weight when `device() != Cpu`); +16/−2 lines. Pod parity + re-profile is the gate (no local GPU). This is Lever 1 of the two-lever backward-offload attack; Lever 2 (pinned+async checkpoint offload pool) follows on the uncontaminated baseline.
