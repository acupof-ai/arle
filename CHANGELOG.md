...
- **DEFAULT FLIP — FA3 decode split ceiling is derived from the SM count: −11.2% decode step at batch 1** (2026-08-04; `574045dc1`+`53f9c5143`+`0e750fa18`, [win](docs/experience/wins/2026-08-04-fa3-decode-splits-fill-the-sms.md)). `--qwen35-fa3-decode-splits` is an upper bound on FA3's own `num_splits_dynamic`, and the shipped constant 8 bound it at batch 1: `pack_gqa` leaves `kv_heads × splits` = 32 work tiles for H20's 78 SMs. The default is now `0` = derive, `sm_count / (batch × kv_heads)` floored at 8 — 20 at batch 1, unchanged at batch ≥ 4. 33K context, W8A16 27B, two reps: batch 1 18.989 → 16.859 ms, batch 4 and 8 inside a 0.09 ms noise floor measured on identical configurations. Needle ladder to 33691 prompt tokens, both arms `exact=3 miss=0` at every rung. The kernel remains 8× off its 618 µs roofline; DRAM traffic is the next probe.
