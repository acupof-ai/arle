...
- **DEFAULT FLIP — `--qwen35-gdr-chunked` defaults ON with a runtime availability probe** (2026-08-02; `2e2ab667c`). License: needle ladder 1k/4k/8k ×3 both arms 9/9 exact + deterministic; 33K cold prefill −26/−28%; stub-build + default flags degrades silently to the recurrent scan (one-shot seq_len=0 probe, FA3 link-marker pattern — fallback log ×1, zero engine errors); real build runs chunked by default (8k cold 3.6 s vs 5.1 s).
