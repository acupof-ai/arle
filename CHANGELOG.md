...
- **Weight axis (architecture refactor step 5) — phase exit.** The quant-format ABI and the weight-layout planner now live in host-only `infer-quant`: `plan_weight_layout(query, caps, policy)` is the single site where format + shape + SM-tier gates decide a weight's resident layout, and the device-side repack fns execute unconditionally for a matrix with its source buffers present. NVFP4 stays Required (hard error below sm_80 or on a shape the Marlin kernel is not instantiated for); W8A16 and per-channel FP8 stay Optional and demote to the scalar/dequant path. The one device probe (DeepGEMM native preflight) enters as a host policy bool; `cargo tree -p infer-quant` has no `cuda-kernels`. 11 files, +585/−176. See [wins 2026-09-10](docs/experience/wins/2026-09-10-weight-layout-planner.md).

- **Examples now have a compile gate.** `--lib`/`--tests` never compile `examples/`, so 28 example files in the tree had no gate at all. `metal_kv_memory_probe` now builds in metal-ci.yml and the pre-push Metal section; autograd/train CPU examples (27) are covered by switching the CI Lint + pre-push clippy lines to `--all-targets`. The 8 infer-cuda CUDA examples cannot link on Mac/CI and need a `clippy --examples` gate after their lint debt is cleared — tracked separately.
