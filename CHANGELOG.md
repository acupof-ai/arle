...
- **FEAT (accept) — `--lora-merge-fp8`: 27B all-linear LoRA merge fits one GPU** (2026-08-16; `cd5d9afd1`, `ed945fde1`, [wins](docs/experience/wins/2026-08-16-lora-merge-requant-fp8.md)). Merged weights are requantized back into the FP8 serving slots per layer (new `quantize_bf16_to_fp8_block_scaled_cuda`, dual of the existing dequant; `DeviceMatrix.pristine_fp8` keeps the base pair at unchanged device addresses so share-frozen-base aliases and the idempotent re-merge keep working). Residency drops from ~3× base (retired FP8 + merged BF16, which OOMed at layer 59) to ~2× base. Verified single-GPU 27B FP8 `all-linear`, judge-free, 2 rounds: trained 6 rollouts per round (loss 0.0542 → 0.0653), exit 0. Default off; one extra FP8 quantization error on merged weights. Known limit: an in-process 27B judge still cannot be co-resident after training — use a second GPU or `--self-consistency`.

- **FIX (accept) — OPD 65536 long-seq OOM: cached teacher hidden + per-layer cleanup** (2026-08-16; `cd9784f6c`, [wins](docs/experience/wins/2026-08-16-opd-65536-longseq-oom-fix.md)). The windowed KL path re-ran the teacher forward on a growing prefix per window; the last window processed the full 65 543-token sequence and OOM'd on linear-attention scratch (~3 GB/layer × 18 layers). Teacher now runs one full-seq forward to hidden states; per-window logits derive from the cached hidden. `forward_hidden_freeing_intermediates` prunes per-layer scratch from the store. `device_synchronize` before `trim_memory_pool` drains all streams. `max_seq_len` compensated 8/7 for the gen_reserve clamp. Verified: step 1/1 at 65536+8 on qwen35-08b, loss 4.334862, peak ~26 GB.
