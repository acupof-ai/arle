...
## [Unreleased]
- **PERF/CORRECTION — device MoE group planning + indirect dispatch: the ids fence is GONE (three planner kernels build the expert-major plan on GPU, `vkCmdDispatchIndirect` drives the class GEMVs, ids never visit the host), gated by device-plan == host-plan ELEMENT-FOR-ELEMENT on randomized and 16 real router rosters, with truncated AND full-scale prefill=decode still 0.000e0. The receipts correct their own premise: the "ids fence = 80% of the wall" figure was GPU work DRAINING at the fence (the profiler books drained GPU-busy against the flushing stage), so true recovery at chunk 256 is +3.2% (62.7 → 64.7 tok/s, same load) — what actually died is the chunk-count pathology (chunk 64: 0.6 → 58.7 tok/s) and the structure itself. The fence-free verify chunk reads the same 69-123 ms/cycle, so speculation stays shelved for the same reason: the residual is kernel-side (NVFP4 dequant-ALU floor, linattn occupancy, GEMV-vs-GEMM shape). ALSO: `powercfg` read Balanced this day — every 2026-08-28 absolute is a Balanced-mode number and the "Performance mode" labels in that day's commits were unverified; the in-sitting ratios, including ARLE-vs-llama.cpp, stand** (2026-08-28; [wins](docs/experience/wins/2026-08-28-qwen4exp-device-moe-plan-fence-free-prefill.md))
- **FEAT + VERDICT — device-side MoE group planning: the prefill's per-(layer,chunk) ids fence is GONE (default; `ARLE_QWEN4_PREFILL_HOST_PLAN=1` keeps the fenced host oracle), and the A/B corrects the receipt that motivated it. Three new planner kernels (atomic counts → one-thread deterministic scan → closed-form emit) rebuild `plan_moe_groups`' plan ON DEVICE, element-for-element equal to the host oracle (new gate: randomized + 16 real-router rosters, poisoned buffers, mutation-killed); the class-specialized grouped GEMVs take their y extent from device-built `VkDispatchIndirectCommand`s via new `vulkan-sys` `dispatch_indirect`/`barrier_indirect` plumbing — no GEMV shader change (`ne11` is an identity modulus, so the fixed-region CAPACITY is a valid record-time bound). Load-bearing trick: fixed worst-case class regions (`MoePlanLayout`, ~6x live pairs, ~330 MB host-cached) keep every descriptor offset record-time. Bit-exact gate 0.000e0 (device mode, host mode, truncated AND full-scale parity: max abs 0.000e0, argmax equal). Measured same-load A/B (chunk 256, 512 tok, Balanced powercfg — not the Performance fingerprint): 62.7 → 64.7 tok/s (+3.2%); chunk 64 fence-free 58.7 tok/s where the old receipt read 0.6. THE CORRECTION: the "ids fence = 80% of wall" reading double-counted — the fence phase was where recorded GPU work DRAINED; removing it moves 6.45 s of wall to the end-of-chunk flush and recovers only the ~0.24 s of true fence overhead plus the chunk-count pathology. k=1 speculative probe confirms the same for verify chunks (69-123 ms/cycle, 1.08x best — unchanged): both walls are GPU execution, so the next prefill levers are kernel-side (NVFP4 dequant ALU, linattn occupancy), not fence-side** (2026-08-28; [wins](docs/experience/wins/2026-08-28-qwen4exp-device-moe-plan-fence-free-prefill.md))
- **VERDICT — MTP speculative decode post-merge re-measure: losslessness HOLDS on the merged tree (gate green, sequences equal), but the +10-15% did NOT survive the faster mainline — best case is code k=1 at 1.09×, the rest regress (0.52-0.92×), because the decode loop got the fence-free/resident/pipelined structure and `forward_verify` (grouped-prefill machinery) did not: a 2-position verify chunk still pays 48 per-layer ids fences (68-119 ms vs the 87 ms two-token baseline). Speculation ships lossless-but-shelved as a perf feature, and the three measured walls — prefill's 80% ids fence, the verify overhead, MTP's payoff — are now ONE named lever: device-side expert grouping (router + top-k + group plan on GPU, indirect dispatch, ids never visiting the host)** (2026-08-28)
- **FEAT — qwen4_exp MTP speculative decode: greedy-LOSSLESS at full scale (speculative output == plain greedy decode token-for-token over 3 prompt classes x 5 draft depths x 40 tokens, 15/15), acceptance IN the vendor's band (57.9-71.9% per step at k=2 vs their 50-70%), best measured k=1 at +10-15% effective throughput (46.0/46.3/49.6 vs 50.6/51.1/56.8 ms/token, same sitting) — the vendor's own +13-15%, one depth shallower than their k=2 cap. The head is the checkpoint's own `mtp.*`: PRE-mixer 10240-wide conditioning (pinned by the `pre_fc_norm_hidden` [10240] shape), recurrent drafting, 1536 per-expert BF16 slices of the quant-excluded stacked layout that spill cold-first. Rollback is TWO device-to-device buffer copies (GDN S + conv rings, 117 MB; the save rides the verify submit, so only a rejection pays) plus host clones of the PLE ring and n-gram window; KV is positional, so winding `seq_len` back IS its rollback — measured 0.19-1.90 ms/cycle, under 4% of a token at the worst depth. The gate is proven able to fail: `ARLE_QWEN4_SPEC_FAULT=skip-gdn|skip-ple|skip-ngram` breaks exactly its own piece of the state comparison. NOT a default flip (chat regresses at every k>1); ships as `generate_speculative` + a `Qwen4DraftSource` trait. Two premises corrected by measurement: the verify amortizes the dense tier only 1.31x at k=1 (the 512-expert union grows ~linearly in k and the per-position dispatch floor does not batch — that, not rollback, is where the win goes), and a first sweep's baselines were contaminated by a sibling agent's full-scale bench on the same 63.6 GB box and re-measured exclusively. Bonus, measured on the shipping path the verify machinery reuses: the generalized NUM_COLS<=8 dense batching is +18.4% PREFILL in a matched same-load A/B (68.2 vs 57.6 tok/s at chunk 256, 512 tok), bit-exactness preserved** (2026-08-28; [wins](docs/experience/wins/2026-08-28-qwen4exp-mtp-speculative-decode.md))
- **FEAT — chunked WY-form GDN prefill lane (study lift #3), verdict: OPT-IN ONLY, and the A/B re-ranked the prefill wall. Two new kernels (per-(64-chunk, head) intra kernel with in-place triangular solve; serial-in-chunks state pass against the same resident f32 state), device parity ≤1.5e-4 vs the serial oracle, default path bit-exact-untouched. Full-scale A/B: 50.3 → 55.8 tok/s (+11%) — but 48 layers of 512-expert routers amplify the reassociation into 23% expert-set flips and a final argmax change (identical failure mode to the GEMM lane), so `ARLE_QWEN4_PREFILL_CHUNKED_GDN=1` stays gated with the receipt printed by the A/B itself. The bigger finding: `pf.moe.ids_fence` is 80% of the chunk-256 prefill wall (7.46 of 9.27 s) — the fence STRUCTURE, not any kernel under it, is the next prefill lever** (2026-08-28; [wins](docs/experience/wins/2026-08-28-qwen4exp-chunked-gdn-prefill.md))
- **PERF — decode passes same-box llama.cpp: 47.2 → 43.5 ms/token (23.0 tok/s vs their Vulkan tg32 21.96). Resident PLE conv ring (study lift #5 — ring advances in place at a tracked ring_pos; sync-out only for host-canonical consumers; the retired finish_ple flush was ALSO the PLE→hc_pre sync, the same hidden-duty trap the MoE fence had, caught by design this time) + depth-2 submit pipelining (second command buffer + submit_async; the GPU chews a 12-layer batch while the host records the next — submit-wait 39.6 → 11.3 ms). Bit-exact gate 0.000e0 both changes** (2026-08-28)
- **PERF — fence-free MoE decode: 55.5 → 47.2 ms/token (21 tok/s), submits/token 50 → 2. First lift landed from the llama.cpp qwen4exp code-diff study: their fence-free MoE shape, done our way — the SCALE0 GEMV fusion flips from slot-indexed to expert-id-indexed (BIAS0's existing convention) and binds a resident 288 KB weight_scale_2 table, so the router's ids never visit the host and the 48 per-layer flush+readback fences vanish (grouped prefill's per-block scale lists die with them). The deleted flush was secretly also the top-k→GEMV sync; the truncated prefill=decode bit-exact gate caught the missing barrier on the first run. Study's other verdicts banked: D1 GDN l2-norm eps fixed to the reference's 1e-6 (our only confirmed math deviation), D3 resolved in llama.cpp's favor (QSA tail-block +1e9 forcing is reference-exact — S6 lift spec complete), and the non-findings receipts: their qwen4exp has no MTP and no recurrent rollback. The "their Vulkan dies at QSA's top_k" claim did NOT survive the same-box baseline (llama-bench, built from master source, same sitting): pp128 187.1 / tg32 22.0 tok/s vs ARLE's 56 / 21.2 — decode is a 3.5% TIE on Vulkan, prefill a 3.3× gap ranked exactly where the lift list pointed (chunked delta-net port launched same day). Wall is now ~85% GPU-execution (40 of 47 ms)** (2026-08-28)
- **PERF — expert-grouped MoE prefill: 45 → 56 tok/s, BIT-EXACT preserved (0.000e0, bound not relaxed — the expert-major regroup scatters back to token order so decode's slot-order summation survives). 4.7× expert-byte cut, 768→27 dispatches per layer-chunk, zero new kernels (the class-batched MUL_MAT_ID arm carries the SCALE0 fusion plain GemvNvfp4 lacks). The round's own premise corrected by ablation: the fence drain was ~38% expert bytes, not 87 — residual ranked by a new GPU-timestamp drain decomposition (linattn dispatch-bound 2.8s, NVFP4 dequant-ALU floor 1.9s, hc.pre 147k tiny dispatches). plan_moe_groups is the S7 k-token-verify substrate** (2026-08-28)
