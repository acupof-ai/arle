...
- **REJECT (concurrent serving) — DSpark is net-negative once decode is fast** (2026-07-27, `92175f3d5`; [champion row](docs/baselines.md), [repricing](docs/research/2026-07-27-dspark-repriced-after-fa3.md)). 16-session 32k long-agent, 128 req/point, same binary and session, both arms: **+1.5% at c=1, −6.3% at c=8, −7.1% at c=16**. Before FA3 the same drafter was +57.5% at c=1. Acceleration from speculation is a ratio against the step it replaces — FA3 cut that step 2.7×, and the verify cost is no longer covered. Keep DSpark available for c=1 latency; stop spending on it for throughput. Also: the machine saturates at c=8 (+1.0% total tok/s to c=16 while ITL p90 reaches 8.1 s), so the remaining throughput is in the scheduler, not the kernels.
