...
- **PERF — Vulkan prefill 3.45 → 37.0 tok/s (10.7×) on Qwen3.8-27B-Q4_K_M / Strix Halo: batched `mul_mmq` chunks replace the per-token GEMV loop (6.9×), then a KHR cooperative-matrix `mul_mm` adds 1.57×. llama.cpp's `l_warptile_mmq` is the *worst* of eleven tiles here (0.57× geomean, 0.20× at n=32) — the limit is per-subgroup accumulator VGPRs, not warp width, so cap `WM=WN=32` and buy tile area with more subgroups instead. Bottleneck is now the token-serial gated-delta recurrence (38% of a 256-token chunk)** (2026-08-20; [wins](docs/experience/wins/2026-08-20-vulkan-coopmat-prefill-warptile.md))
